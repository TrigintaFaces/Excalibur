...
| `TagCardinalityGuard` | 6 | Various transport packages | **Review** -- may need shared base in Transport.Abstractions |
...
| Class Name | Count | Namespaces | Recommendation |
|------------|-------|-----------|----------------|
| `RetryPolicy` | 1 | Resilience.Polly (executor) | **RESOLVED in S687** -- 4 config POCOs renamed: `BatchRetryOptions`, `SagaRetryOptions`, `RabbitMqRetryOptions`, `PubSubRetryOptions`. `Dispatch/Configuration/RetryPolicy` retired as `DispatchRetryOptions`. See ADR-251 D.2. |
...
| `SqlServerConnectionOptions` | 2 | **Review** -- potential consolidation |
| `SqlServerPoolingOptions` | 2 | **Review** -- potential consolidation |
| `SessionOptions` | 2 | **Keep** -- different session contexts |
...
- **~10 warrant review** for potential shared abstractions (TagCardinalityGuard, RetryPolicy, DeadLetterOptions)
...
4. **TagCardinalityGuard**: Evaluate if 6 copies can share a single implementation in Transport.Abstractions
