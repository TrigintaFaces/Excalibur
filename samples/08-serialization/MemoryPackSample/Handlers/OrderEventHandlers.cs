...
namespace MemoryPackSample.Handlers;
...
/// </summary>
public sealed class OrderCompletedEventHandler : IEventHandler<OrderCompletedEvent>
{
	private readonly ILogger<OrderCompletedEventHandler> _logger;

	/// <summary>
	/// Initializes a new instance of the <see cref="OrderCompletedEventHandler"/> class.
	/// </summary>
	public OrderCompletedEventHandler(ILogger<OrderCompletedEventHandler> logger) => _logger = logger;

	/// <inheritdoc/>
	public Task HandleAsync(OrderCompletedEvent message, CancellationToken cancellationToken)
	{
		_logger.LogInformation(
			"[Handler] Order completed: {OrderId}, Final Amount: {Amount:C}, Completed At: {CompletedAt}",
			message.OrderId,
			message.FinalAmount,
			message.CompletedAt);
