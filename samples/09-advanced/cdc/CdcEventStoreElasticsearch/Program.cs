...
// Register event types so the secure-default serializer resolves them without the scan.
...
// view-id + position-tracking pattern. The framework catches-up each
// view builder independently from the last saved position.

builder.Services.AddMaterializedViews(views =>
{
	views.AddBuilder<CustomerOrderCountView, CustomerOrderCountViewBuilder>()
		 .UseElasticSearch(options =>
		 {
			 options.NodeUri = elasticsearchNodeUris[0];
		 });
});
