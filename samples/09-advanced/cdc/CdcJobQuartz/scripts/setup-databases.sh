...
# ============================================================================
# CDC Job with Quartz.NET Sample - Database Setup Script
# ============================================================================
#
...
# (port 1433) and Section 2 targets the event store (port 1434). Each section is applied to
# its own server below, split at the Section 2 banner.
...
echo "============================================================================"
echo "CDC Job with Quartz.NET Sample - Database Setup"
echo "============================================================================"
echo ""

# Check if containers are running
echo "Checking Docker containers..."
if ! docker ps | grep -q excalibur-sqlserver-cdc-job; then
    echo "ERROR: SQL Server #1 (CDC Source) container is not running."
    echo "Please run: docker-compose up -d"
    exit 1
fi

if ! docker ps | grep -q excalibur-sqlserver-eventstore-job; then
...
until docker exec excalibur-sqlserver-cdc-job /opt/mssql-tools18/bin/sqlcmd \
...
until docker exec excalibur-sqlserver-eventstore-job /opt/mssql-tools18/bin/sqlcmd \
    -S localhost -U sa -P "$SA_PASSWORD" -C -Q "SELECT 1" > /dev/null 2>&1; do
    echo "  Waiting..."
    sleep 2
done
...
echo "============================================================================"
echo "Applying Section 1 of setup-databases.sql (LegacyDb, CDC, LegacyCustomers)..."
...
echo "============================================================================"
echo "Applying Section 2 of setup-databases.sql (EventStore database and tables)..."
...
docker exec excalibur-sqlserver-cdc-job /opt/mssql-tools18/bin/sqlcmd \
...
echo ""
echo "Event-store tables on SQL Server #2:"
docker exec excalibur-sqlserver-eventstore-job /opt/mssql-tools18/bin/sqlcmd \
    -S localhost -U sa -P "$SA_PASSWORD" -C -d EventStore \
    -Q "SELECT s.name + '.' + t.name AS TableName
        FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
        WHERE s.name + '.' + t.name IN
            ('dbo.EventStoreEvents', 'dbo.EventStoreSnapshots', 'dbo.IdentityMap', 'Cdc.CdcProcessingState')" \
    -W
