---
sidebar_position: 1
title: Compliance Checklists
description: Detailed compliance checklists for FedRAMP, SOC 2, GDPR, and HIPAA
---

# Compliance Checklists

Step-by-step checklists for achieving and maintaining compliance with major regulatory frameworks.

## Available Checklists

| Framework | Description | Status |
|-----------|-------------|--------|
| [FedRAMP](fedramp.md) | Federal Risk and Authorization Management Program | 14/14 Controls |
| [SOC 2](soc2.md) | Service Organization Control 2 Trust Services Criteria | Complete |
| [GDPR](gdpr.md) | General Data Protection Regulation | Complete |
| [HIPAA](hipaa.md) | Health Insurance Portability and Accountability Act | Complete |

## Using These Checklists

Each checklist provides:

1. **Control Requirements** - What the regulation requires
2. **Implementation Guidance** - How to implement with Dispatch/Excalibur
3. **Evidence Requirements** - What documentation you need
4. **Automation Scripts** - Automated evidence collection where available

## Related Documentation

- [Compliance Overview](../index.md) - Introduction to compliance capabilities
- [Quick Start](../quick-start.md) - 30-minute compliance setup guide
- [Navigation Guide](../navigation-guide.md) - How to navigate compliance docs

## See Also

- [Compliance Overview](../index.md) - Compliance framework capabilities and architecture
- [Quick Start](../quick-start.md) - 30-minute compliance setup guide
- [GDPR Checklist](gdpr.md) - GDPR certification readiness checklist
- [HIPAA Checklist](hipaa.md) - HIPAA certification readiness checklist
