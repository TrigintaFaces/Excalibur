...
// AOT-safe: use AddDiscoveredHandlers() instead of AddHandlersFromAssembly()
...
`AddDiscoveredHandlers()` is generated by `HandlerRegistrySourceGenerator` and performs explicit, AOT-safe DI registration of all discovered handlers — zero reflection, zero assembly scanning. `AddGeneratedServices()` is generated by `ServiceRegistrationSourceGenerator` and contains explicit `services.AddScoped<THandler>()` calls for every `[AutoRegister]` type.
