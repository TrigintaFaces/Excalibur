...
public interface IOutbox
{
	...
	Task TryUnReserveOneRecordsAsync(string dispatcherId, CancellationToken cancellationToken);
	...
	Task<IEnumerable<OutboxRecord>> TryReserveOneRecordsAsync(
		string dispatcherId,
		int batchSize,
	...
	Task<int> DispatchReservedRecordAsync(string dispatcherId, OutboxRecord record, Func<OutboxRecord, Task<int>> dispatch);
	...
	Task<int> SaveEventsAsync<TKey>(IAggregateRoot<TKey> aggregate, IReadOnlyDictionary<string, string>? messageHeaders,
		string? destination);
	...
	Task<int> SaveMessagesAsync(IEnumerable<OutboxMessage> messages);
}
