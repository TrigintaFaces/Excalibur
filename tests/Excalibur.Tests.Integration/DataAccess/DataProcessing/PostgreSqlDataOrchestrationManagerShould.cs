...
public class PostgreSqlDataOrchestrationManagerShould(PostgreSqlContainerFixture fixture, ITestOutputHelper output)
	: PostgreSqlHostTestBase(fixture, output)
{
	...
	private readonly TaskCompletionSource _beforeDeleteTcs = new(TaskCreationOptions.RunContinuationsAsynchronously);
	...
	public async Task ShouldUpdateCompletedCount()
	{
		...
		var completedCount = await DataProcessingMother.GetCompletedCount(taskId, connection).ConfigureAwait(true);
