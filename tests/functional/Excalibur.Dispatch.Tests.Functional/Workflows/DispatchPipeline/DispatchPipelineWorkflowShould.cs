...
namespace Excalibur.Dispatch.Tests.Functional.Workflows.DispatchPipeline;

/// <summary>
/// End-to-end functional tests for the Dispatch pipeline workflow.
...
public sealed class DispatchPipelineWorkflowShould : FunctionalTestBase
{
	/// <summary>
	...
	public async Task ExecuteMiddlewareInCorrectOrder()
	{
		...
		var steps = executionLog.Steps.ToList();
	...
	/// Uses ConcurrentQueue to preserve insertion order (ConcurrentBag doesn't).
	/// </summary>
	public sealed class ExecutionLog
	{
		...
		public void Log(string step) => _steps.Enqueue(step);
	}

	/// <summary>
	/// Simple in-memory cache simulation.
	/// </summary>
	public sealed class SimulatedCache
	{
	...
	/// </summary>
	public sealed class SimulatedPipeline
	{
	...
	public sealed record TraceableCommand(string TraceId);
	public sealed record FailingCommand(string CommandId, int FailCount);
