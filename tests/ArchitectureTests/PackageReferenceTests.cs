...
public sealed class PackageReferenceTests
{
    ...
    public void DispatchCsproj_MustNotReference_Dapper()
    {
        ...
        {
            true.ShouldBeTrue("Excalibur.Dispatch.csproj not found at expected location.");
            return;
        }
    ...
    public void ExcaliburDataSqlServerCsproj_ShouldReference_DatabasePackages()
    {
        ...
        {
            true.ShouldBeTrue("Excalibur.Data.SqlServer.csproj not found.");
            return;
        }
    ...
    public void ExcaliburDataPostgresCsproj_ShouldReference_DatabasePackages()
    {
        ...
        {
            true.ShouldBeTrue("Excalibur.Data.Postgres.csproj not found.");
            return;
        }
    ...
    private static string FindDispatchCsproj()
    {
        ...
        while (currentDir != null)
        {
            ...
            {
                var dispatchCsproj = Path.Combine(currentDir.FullName, "src", "Dispatch", "Excalibur.Dispatch", "Excalibur.Dispatch.csproj");
                if (File.Exists(dispatchCsproj)) return dispatchCsproj;
                dispatchCsproj = Path.Combine(currentDir.FullName, "src", "Dispatch", "Excalibur.Dispatch.csproj");
                if (File.Exists(dispatchCsproj)) return dispatchCsproj;
            }
    ...
    private static string FindExcaliburDataSqlServerCsproj()
    {
        ...
        while (currentDir != null)
        {
            var solutionFile = Path.Combine(currentDir.FullName, "Excalibur.sln");
            if (File.Exists(solutionFile))
                return Path.Combine(currentDir.FullName, "src", "Excalibur", "Excalibur.Data.SqlServer", "Excalibur.Data.SqlServer.csproj");
            currentDir = currentDir.Parent;
        }
        return string.Empty;
    }
    ...
    private static string FindExcaliburDataPostgresCsproj()
    {
        ...
        while (currentDir != null)
        {
            var solutionFile = Path.Combine(currentDir.FullName, "Excalibur.sln");
            if (File.Exists(solutionFile))
                return Path.Combine(currentDir.FullName, "src", "Excalibur", "Excalibur.Data.Postgres", "Excalibur.Data.Postgres.csproj");
            currentDir = currentDir.Parent;
        }
        return string.Empty;
    }
    ...
    {
        if (!File.Exists(csprojPath)) return Enumerable.Empty<string>();
        try
        {
            var doc = XDocument.Load(csprojPath);
            return doc.Descendants()
