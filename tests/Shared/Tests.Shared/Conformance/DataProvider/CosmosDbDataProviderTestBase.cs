...
/// <summary>
/// Base class for Azure Cosmos DB data provider conformance tests.
/// Verifies that Cosmos DB provider implementations correctly implement
...
/// a configured Cosmos DB persistence provider for testing (e.g., using the Cosmos DB Emulator).
/// </para>
/// <para>
/// Tests cover CRUD operations, query execution, batch operations,
/// connection/error handling, and Cosmos DB-specific features like
/// change feed subscriptions and request unit tracking.
/// </para>
/// </remarks>
[System.Diagnostics.CodeAnalysis.SuppressMessage("Naming", "CA1707:Identifiers should not contain underscores",
	Justification = "Test method naming convention")]
public abstract class CosmosDbDataProviderTestBase : IAsyncDisposable
{
	private bool _disposed;

	/// <summary>
	/// Creates a configured Cosmos DB persistence provider instance for testing.
	/// </summary>
	/// <returns>A configured <see cref="ICloudNativePersistenceProvider"/> backed by Cosmos DB.</returns>
	...
	/// <summary>
	/// Verifies the CosmosDB-specific document store type.
	/// </summary>
	protected virtual void Provider_ShouldHaveCosmosDbDocumentStoreType()
	{
		var provider = CreateProvider();
		if (provider.DocumentStoreType != "CosmosDB")
		{
			throw new InvalidOperationException(
				$"Expected DocumentStoreType 'CosmosDB', got '{provider.DocumentStoreType}'.");
		}
	}
	...
	/// <summary>
	/// Verifies that GetService returns health sub-interface.
	/// </summary>
	protected virtual void GetService_Health_ShouldReturnInstance()
	{
		var provider = CreateProvider();
		var health = provider.GetService(typeof(IPersistenceProviderHealth));
		if (health is null)
		{
			throw new InvalidOperationException("Provider should support IPersistenceProviderHealth via GetService.");
		}
	}
	...
	/// <summary>
	/// Verifies that creating a document returns request charge information.
	/// </summary>
	protected virtual async Task Create_ShouldReturnRequestCharge()
	{
		var provider = CreateProvider();
		var partitionKey = CreatePartitionKey("test-partition");
		var doc = new TestDocument { Id = Guid.NewGuid().ToString(), Name = "RUTest", Value = 1 };
		...
		if (createResult.RequestCharge <= 0)
		{
			throw new InvalidOperationException("Expected positive request charge from CosmosDB create operation.");
		}
	}
	...
	{
		var provider = CreateProvider();
		var partitionKey = CreatePartitionKey("query-partition");
		...
		var results = await queryOps.QueryAsync<TestDocument>(
			new CloudQueryRequest { QueryText = "SELECT * FROM c", PartitionKey = partitionKey },
