...
/// <summary>
/// Real-Firestore-emulator locks on the atomic claim: two claimants polling the same partition receive
/// disjoint sets, and the lease they stamp expires rather than stranding a message.
/// </summary>
/// <remarks>
/// <para>
/// A mocked Firestore client cannot prove any of this. The exclusion is performed by the emulator aborting
/// and re-running a transaction whose document changed underneath it; a mock returns whatever it was told
/// and would certify a store that reads and then writes outside any transaction — which is the shape this
/// store had before, and under which both claimants publish every message.
/// </para>
/// <para>
...
public sealed class FirestoreOutboxStoreClaimAtomicityShould
	: IAsyncLifetime
{
	...
	public FirestoreOutboxStoreClaimAtomicityShould(FirestoreOutboxStoreContainerFixture fixture)
	{
		_fixture = fixture;
	}
	...
	public async Task HandEachMessageToExactlyOneClaimant_WhenTwoClaimantsClaimConcurrently()
	{
		...
		var collection = await CreateCollectionNameAsync().ConfigureAwait(false);
		var partitionKey = new PartitionKey($"pk-{Guid.NewGuid():N}");

		var stagingStore = await CreateStoreAsync(collection, leaseTimeoutSeconds: 120).ConfigureAwait(false);
		...
		var claimantA = await CreateStoreAsync(collection, leaseTimeoutSeconds: 120).ConfigureAwait(false);
		var claimantB = await CreateStoreAsync(collection, leaseTimeoutSeconds: 120).ConfigureAwait(false);
		...
		duplicates.ShouldBeEmpty(
			"Two concurrent claimants received the same message. The claim must read and write inside one "
			+ "transaction: a read followed by a separate write cannot exclude a competitor that read the "
			+ "same state, so both claimants succeed and both publish.");
	...
	public async Task RefuseToReclaimAMessage_WhileItsLeaseIsStillLive()
	{
		...
		var store = await CreateStoreAsync(collection, leaseTimeoutSeconds: 120).ConfigureAwait(false);
		var message = CreateMessage(partitionKey.Value);
	...
	public async Task ReclaimAMessage_AfterItsLeaseExpires()
	{
		...
		var store = await CreateStoreAsync(collection, leaseTimeoutSeconds: 1).ConfigureAwait(false);
		var message = CreateMessage(partitionKey.Value);
		_ = await store.AddAsync(message, partitionKey, ct).ConfigureAwait(false);
	...
	private Task<string> CreateCollectionNameAsync()
	{
	...
	private async Task<FirestoreOutboxStore> CreateStoreAsync(string collection, int leaseTimeoutSeconds)
	{
