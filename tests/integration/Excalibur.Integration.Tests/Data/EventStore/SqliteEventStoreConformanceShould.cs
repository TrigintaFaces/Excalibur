...
/// real SQLite, through the SQLite provider's OWN registration extension.
/// </summary>
/// <remarks>
/// <para>
...
/// built by <c>UseSqlite</c>, so what every arm asserts against is the object the shipped registration
...
/// </para>
/// <para>
/// SQLite is an embedded, file-based database, so these arms run against real infrastructure with no
/// container and are inherently non-skipped. The store auto-creates its table on first use.
/// </para>
/// </remarks>
...
public sealed class SqliteEventStoreConformanceShould : EventStoreConformanceTestKit,
	IClassFixture<SqliteEventStoreFixture>
{
	...
	public SqliteEventStoreConformanceShould(SqliteEventStoreFixture fixture) => _fixture = fixture;
	...
	protected override void ConfigureProvider(
		...
		=> _ = services.AddExcalibur(x => x.AddEventSourcing(es => es.UseSqlite(sqlite =>
		{
			sqlite.ConnectionString = _fixture.ConnectionString;
			sqlite.EventTypeInfoResolver = eventTypeInfoResolver;
		})));
