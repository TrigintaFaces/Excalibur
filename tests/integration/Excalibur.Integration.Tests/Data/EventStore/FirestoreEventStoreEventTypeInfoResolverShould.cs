...
/// <summary>
/// Locks the seam that carries the host's source-generated JSON type-info resolver through the Firestore
...
/// </para>
/// <para>
/// Against the real emulator and through <c>UseFirestore</c>, not by constructing the store by hand: a
/// store that accepts a resolver nobody hands it is still broken for every consumer, so the registration is
/// the half that matters. The Firestore builder exposes no resolver method of its own, so the consumer
/// surface is a <c>Configure</c> on the shipped options type - which is what this suite exercises,
/// alongside the builder rather than instead of it. The emulator is required, never skip-gated: an arm that
/// passes by not running is the gap that ships the bug.
/// </para>
/// <para>
/// Every arm addresses its own aggregate by a fresh identifier, so this suite needs no collection cleanup
/// and is unaffected by which collection the provider writes to.
/// </para>
/// <para>
...
public sealed class FirestoreEventStoreEventTypeInfoResolverShould : IAsyncLifetime
{
	private const string AggregateType = "FsResolverAggregate";

	private readonly FirestoreEventStoreContainerFixture _fixture;

	public FirestoreEventStoreEventTypeInfoResolverShould(FirestoreEventStoreContainerFixture fixture) =>
		_fixture = fixture;

	public ValueTask InitializeAsync()
	{
		_fixture.DockerAvailable.ShouldBeTrue(
			"The resolver seam is verified against a real Firestore emulator, through the provider's own "
	...
	public async Task Serialize_ThroughReflection_WhenNoResolverIsConfigured()
	{
		...
		var result = await store.AppendAsync(
			aggregateId,
			AggregateType,
			[new UndeclaredFsTestEvent { AggregateId = aggregateId }],
			expectedVersion: -1,
	...
	public async Task Write_ByteIdenticalPayloads_WithAndWithoutAResolver()
	{
		...
		var withResolver = await AppendAndLoadAsync(FsResolverTestEventContext.Default).ConfigureAwait(false);
	...
	{
		var stored = await AppendAndLoadAsync(FsResolverTestEventContext.Default).ConfigureAwait(false);

		// Read back through the canonical reflection options a differently-configured host would use.
		var roundTripped = JsonSerializer.Deserialize<FsResolverTestEvent>(
		...
		roundTripped.Shade.ShouldBe(FsResolverTestShade.Green);
	...
	private ServiceProvider BuildProvider(IJsonTypeInfoResolver? resolver)
	{
		...
		{
			// The consumer surface: the Firestore builder carries no resolver method, so the resolver is set
			...
			_ = services.Configure<FirestoreEventStoreOptions>(options => options.EventTypeInfoResolver = resolver);
		}
	...
	private static FsResolverTestEvent CreateEvent(string aggregateId) => new()
	{
		EventId = "e-1",
		AggregateId = aggregateId,
		OccurredAt = new DateTimeOffset(2026, 1, 2, 3, 4, 5, TimeSpan.Zero),
		Name = "order-placed",
		Shade = FsResolverTestShade.Green,
...
internal enum FsResolverTestShade
{
	Red,
	Green,
}
...
internal sealed class FsResolverTestEvent : IDomainEvent
{
	...
	public FsResolverTestShade Shade { get; set; }
...
/// </remarks>
[JsonSerializable(typeof(FsResolverTestEvent))]
[JsonSerializable(typeof(string))]
[JsonSerializable(typeof(int))]
[JsonSerializable(typeof(bool))]
internal sealed partial class FsResolverTestEventContext : JsonSerializerContext;
