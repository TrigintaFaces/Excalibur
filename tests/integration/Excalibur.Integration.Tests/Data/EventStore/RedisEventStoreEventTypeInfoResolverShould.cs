...
/// <summary>
/// Locks the seam that carries the host's source-generated JSON type-info resolver through the Redis
...
/// </para>
/// <para>
/// Against a real Redis and through <c>UseRedis</c>, not by constructing the store by hand: a store that
/// accepts a resolver nobody hands it is still broken for every consumer, so the registration is the half
/// that matters. The Redis builder exposes no resolver method of its own, so the consumer surface is a
/// <c>Configure</c> on the shipped options type - which is exactly what this suite exercises, alongside the
/// builder rather than instead of it. The container is required, never skip-gated: an arm that passes by
/// not running is the gap that ships the bug.
/// </para>
/// <para>
...
public sealed class RedisEventStoreEventTypeInfoResolverShould
	: IClassFixture<RedisEventStoreContainerFixture>, IAsyncLifetime
{
	private const string AggregateType = "RedisResolverAggregate";

	private readonly RedisEventStoreContainerFixture _fixture;

	public RedisEventStoreEventTypeInfoResolverShould(RedisEventStoreContainerFixture fixture) =>
		_fixture = fixture;

	public ValueTask InitializeAsync()
	{
		_fixture.DockerAvailable.ShouldBeTrue(
			"The resolver seam is verified against a real Redis, through the provider's own registration. "
	...
	public async Task Serialize_ThroughReflection_WhenNoResolverIsConfigured()
	{
		...
		var result = await store.AppendAsync(
			aggregateId,
			AggregateType,
			[new UndeclaredRedisTestEvent { AggregateId = aggregateId }],
			expectedVersion: -1,
	...
	public async Task Write_ByteIdenticalPayloads_WithAndWithoutAResolver()
	{
		...
		var withResolver = await AppendAndLoadAsync(RedisResolverTestEventContext.Default).ConfigureAwait(false);
	...
	{
		var stored = await AppendAndLoadAsync(RedisResolverTestEventContext.Default).ConfigureAwait(false);

		// Read back through the canonical reflection options a differently-configured host would use.
		var roundTripped = JsonSerializer.Deserialize<RedisResolverTestEvent>(
		...
		roundTripped.Shade.ShouldBe(RedisResolverTestShade.Green);
	...
	private ServiceProvider BuildProvider(IJsonTypeInfoResolver? resolver)
	{
		...
		_ = services.AddExcalibur(x => x.AddEventSourcing(es => es.UseRedis(redis =>
			_ = redis.ConnectionString(_fixture.ConnectionString))));

		if (resolver is not null)
		{
			// The consumer surface: the Redis builder carries no resolver method, so the resolver is set on
			...
			_ = services.Configure<RedisEventStoreOptions>(options => options.EventTypeInfoResolver = resolver);
		}
	...
	private static RedisResolverTestEvent CreateEvent(string aggregateId) => new()
	{
		EventId = "e-1",
		AggregateId = aggregateId,
		OccurredAt = new DateTimeOffset(2026, 1, 2, 3, 4, 5, TimeSpan.Zero),
		Name = "order-placed",
		Shade = RedisResolverTestShade.Green,
...
internal enum RedisResolverTestShade
{
	Red,
	Green,
}
...
internal sealed class RedisResolverTestEvent : IDomainEvent
{
	...
	public RedisResolverTestShade Shade { get; set; }
...
/// </remarks>
[JsonSerializable(typeof(RedisResolverTestEvent))]
[JsonSerializable(typeof(string))]
[JsonSerializable(typeof(int))]
[JsonSerializable(typeof(bool))]
internal sealed partial class RedisResolverTestEventContext : JsonSerializerContext;
