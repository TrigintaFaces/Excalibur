...
/// <summary>
/// Locks the seam that carries the host's source-generated JSON type-info resolver through the Cosmos DB
...
/// </para>
/// <para>
/// Against the real emulator and through <c>UseCosmosDb</c>, not by constructing the store by hand: a store
/// that accepts a resolver nobody hands it is still broken for every consumer, so the registration is the
/// half that matters. The Cosmos builder exposes no resolver method of its own, so the consumer surface is
/// a <c>Configure</c> on the shipped options type - which is what this suite exercises, alongside the
/// builder rather than instead of it. The emulator is required, never skip-gated: an arm that passes by not
/// running is the gap that ships the bug.
/// </para>
/// <para>
/// The resolver governs the EVENT PAYLOAD only. The Cosmos document that carries that payload is
/// serialized by the consumer-supplied <c>CosmosClient</c>, whose SDK v3 default is Newtonsoft - and the
/// fixture's client deliberately keeps that default. So this suite pins the one boundary the client cannot
/// reach: the bytes the store itself produces before handing the document over.
/// </para>
/// <para>
...
public sealed class CosmosDbEventStoreEventTypeInfoResolverShould
	: IClassFixture<CosmosDbEventStoreContainerFixture>, IAsyncLifetime
{
	private const string AggregateType = "CosmosResolverAggregate";
	...
	public CosmosDbEventStoreEventTypeInfoResolverShould(CosmosDbEventStoreContainerFixture fixture) =>
		_fixture = fixture;

	public ValueTask InitializeAsync()
	{
		_fixture.DockerAvailable.ShouldBeTrue(
			"The resolver seam is verified against a real Cosmos DB emulator, through the provider's own "
			+ "registration. This suite must never be skipped.");
	...
	public async Task Serialize_ThroughReflection_WhenNoResolverIsConfigured()
	{
		...
		var result = await store.AppendAsync(
			aggregateId,
			AggregateType,
			[new UndeclaredCosmosTestEvent { AggregateId = aggregateId }],
			expectedVersion: -1,
	...
	public async Task Write_ByteIdenticalPayloads_WithAndWithoutAResolver()
	{
		...
		var withResolver = await AppendAndLoadAsync(CosmosResolverTestEventContext.Default).ConfigureAwait(false);
	...
	{
		var stored = await AppendAndLoadAsync(CosmosResolverTestEventContext.Default).ConfigureAwait(false);

		// Read back through the canonical reflection options a differently-configured host would use.
		var roundTripped = JsonSerializer.Deserialize<CosmosResolverTestEvent>(
		...
		roundTripped.Shade.ShouldBe(CosmosResolverTestShade.Green);
	...
	private ServiceProvider BuildProvider(IJsonTypeInfoResolver? resolver)
	{
		...
		{
			// The consumer surface: the Cosmos builder carries no resolver method, so the resolver is set on
			// the shipped options type. Registered AFTER the builder deliberately - the builder's own
			...
			_ = services.Configure<CosmosDbEventStoreOptions>(options => options.EventTypeInfoResolver = resolver);
		}
	...
	private static CosmosResolverTestEvent CreateEvent(string aggregateId) => new()
	{
		EventId = "e-1",
		AggregateId = aggregateId,
		OccurredAt = new DateTimeOffset(2026, 1, 2, 3, 4, 5, TimeSpan.Zero),
		Name = "order-placed",
		Shade = CosmosResolverTestShade.Green,
...
internal enum CosmosResolverTestShade
{
	Red,
	Green,
}
...
internal sealed class CosmosResolverTestEvent : IDomainEvent
{
	...
	public CosmosResolverTestShade Shade { get; set; }
...
/// </remarks>
[JsonSerializable(typeof(CosmosResolverTestEvent))]
[JsonSerializable(typeof(string))]
[JsonSerializable(typeof(int))]
[JsonSerializable(typeof(bool))]
internal sealed partial class CosmosResolverTestEventContext : JsonSerializerContext;
