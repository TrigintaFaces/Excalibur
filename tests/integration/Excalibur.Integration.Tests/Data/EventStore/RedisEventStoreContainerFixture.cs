...
/// <summary>
/// Shared fixture for the Redis <see cref="Excalibur.EventSourcing.Redis.RedisEventStore"/> TestContainers.
/// </summary>
/// <remarks>
/// Creates and manages a Redis container for the event store. Redis is schemaless, so no DDL is
/// required. Between tests the fixture issues a <c>FLUSHDB</c> via a dedicated admin connection so each
/// conformance test runs against a clean keyspace; the store under test still connects with the default
/// (non-admin) client surface that consumers use.
/// </remarks>
public sealed class RedisEventStoreContainerFixture : ContainerFixtureBase
{
	...
	protected override async Task InitializeContainerAsync(CancellationToken cancellationToken)
	{
		_container = new RedisBuilder()
			...
			.WithName($"redis-eventstore-test-{Guid.NewGuid():N}")
			.WithCleanUp(true)
			.Build();
