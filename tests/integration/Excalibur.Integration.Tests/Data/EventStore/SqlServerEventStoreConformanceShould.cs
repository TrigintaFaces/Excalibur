...
/// real SQL Server, through the SQL Server provider's OWN registration extension.
/// </summary>
/// <remarks>
/// <para>
...
/// built by <c>UseSqlServer</c>, so what every arm asserts against is the object the shipped registration
/// actually produces. The previous suite constructed the store by hand, which proves it works when handed
/// the right arguments - never that the registration hands them over. It also adds four tenancy arms this
/// provider had no conformance coverage for at all.
/// </para>
/// <para>
/// Against a real SQL Server, not a fake: optimistic-concurrency detection, the authoritative version
...
public sealed class SqlServerEventStoreConformanceShould : EventStoreConformanceTestKit,
	IClassFixture<SqlServerEventStoreContainerFixture>, IAsyncLifetime
{
	...
	public SqlServerEventStoreConformanceShould(SqlServerEventStoreContainerFixture fixture) => _fixture = fixture;

	/// <summary>
	/// Brings the container and its schema up before any arm resolves the provider.
	/// </summary>
	/// <returns>A task that completes when the store's backing infrastructure is ready.</returns>
	/// <remarks>
	...
	public async ValueTask InitializeAsync()
	{
		_fixture.DockerAvailable.ShouldBeTrue(
			"The event-store conformance contract is verified against a real SQL Server - its concurrency, "
	...
	protected override void ConfigureProvider(
		...
		=> _ = services.AddExcalibur(x => x.AddEventSourcing(es => es.UseSqlServer(sql =>
		{
			...
			{
				_ = sql.EventTypeInfoResolver(eventTypeInfoResolver);
			}
		})));
