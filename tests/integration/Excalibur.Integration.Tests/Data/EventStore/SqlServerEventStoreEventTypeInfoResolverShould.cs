...
/// <summary>
/// Locks the seam that carries the host's source-generated JSON type-info resolver through the SQL Server
...
/// </para>
/// <para>
/// Against a real SQL Server and through <c>UseSqlServer</c>, not by constructing the store by hand: a
/// store that accepts a resolver nobody hands it is still broken for every consumer, so the registration is
/// the half that matters. Here that half is load-bearing rather than incidental - the builder writes the
/// resolver onto <c>SqlServerEventSourcingOptions</c> and a separate registration lambda reads it back out
/// and passes it to a constructor parameter, so there are two independent places the resolver can be
/// dropped without any compiler or analyzer noticing. The container is required, never skip-gated: an arm
/// that passes by not running is the gap that ships the bug.
/// </para>
/// <para>
...
public sealed class SqlServerEventStoreEventTypeInfoResolverShould
	: IClassFixture<SqlServerEventStoreContainerFixture>, IAsyncLifetime
{
	private const string AggregateType = "SqlResolverAggregate";

	private readonly SqlServerEventStoreContainerFixture _fixture;

	public SqlServerEventStoreEventTypeInfoResolverShould(SqlServerEventStoreContainerFixture fixture) =>
		_fixture = fixture;

	public async ValueTask InitializeAsync()
	{
		_fixture.DockerAvailable.ShouldBeTrue(
			"The resolver seam is verified against a real SQL Server, through the provider's own "
	...
	public async Task Serialize_ThroughReflection_WhenNoResolverIsConfigured()
	{
		...
		var result = await store.AppendAsync(
			aggregateId,
			AggregateType,
			[new UndeclaredSqlTestEvent { AggregateId = aggregateId }],
			expectedVersion: -1,
	...
	public async Task Write_ByteIdenticalPayloads_WithAndWithoutAResolver()
	{
		...
		var withResolver = await AppendAndLoadAsync(SqlResolverTestEventContext.Default).ConfigureAwait(false);
	...
	{
		var stored = await AppendAndLoadAsync(SqlResolverTestEventContext.Default).ConfigureAwait(false);

		// Read back through the canonical reflection options a differently-configured host would use.
		var roundTripped = JsonSerializer.Deserialize<SqlResolverTestEvent>(
		...
		roundTripped.Shade.ShouldBe(SqlResolverTestShade.Green);
	...
	private ServiceProvider BuildProvider(IJsonTypeInfoResolver? resolver)
	{
		...
		_ = services.AddExcalibur(x => x.AddEventSourcing(es => es.UseSqlServer(sql =>
		{
			_ = sql.ConnectionString(_fixture.ConnectionString)
				.EventStoreSchema(_fixture.SchemaName)
				.EventStoreTable(_fixture.TableName);

			if (resolver is not null)
			{
				_ = sql.EventTypeInfoResolver(resolver);
			}
		})));
	...
	private static SqlResolverTestEvent CreateEvent(string aggregateId) => new()
	{
		EventId = "e-1",
		AggregateId = aggregateId,
		OccurredAt = new DateTimeOffset(2026, 1, 2, 3, 4, 5, TimeSpan.Zero),
		Name = "order-placed",
		Shade = SqlResolverTestShade.Green,
...
internal enum SqlResolverTestShade
{
	Red,
	Green,
}
...
internal sealed class SqlResolverTestEvent : IDomainEvent
{
	...
	public SqlResolverTestShade Shade { get; set; }
...
/// </remarks>
[JsonSerializable(typeof(SqlResolverTestEvent))]
[JsonSerializable(typeof(string))]
[JsonSerializable(typeof(int))]
[JsonSerializable(typeof(bool))]
internal sealed partial class SqlResolverTestEventContext : JsonSerializerContext;
