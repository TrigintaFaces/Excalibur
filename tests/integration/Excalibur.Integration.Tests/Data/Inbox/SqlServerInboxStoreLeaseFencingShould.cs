...
/// <summary>
/// Real-SQL-Server lock for the <b>fenced</b> half of the lease protocol: a caller whose lease has lapsed
/// must not be able to finalize the record of the caller that replaced it.
/// </summary>
/// <remarks>
/// <para>
/// The sibling suite (<see cref="SqlServerInboxStoreLeaseReclaimShould"/>) binds <i>admission</i> — who gets
...
/// </para>
/// <para>
/// <b>Why real infrastructure.</b> The term is the expiry the SERVER resolved from <c>SYSUTCDATETIME()</c>,
/// read back through the MERGE's <c>OUTPUT</c> clause and compared inside the write's own predicate as a
/// real timestamp parameter. A mocked connection returns whatever it was told and can certify neither half
/// — nor the round trip, where the column is <c>DATETIME2(3)</c> (millisecond scale) while the term is
/// encoded with round-trip <c>"O"</c> (7 fractional digits). If that pairing lost or gained a single tick
/// anywhere, every finalization by the legitimate holder would silently fail closed and look exactly like an
/// early expiry — which is what <see cref="CompleteUnderALiveTerm"/> exists to catch.
/// </para>
/// <para>
...
public sealed class SqlServerInboxStoreLeaseFencingShould : IClassFixture<SqlServerInboxStoreContainerFixture>
{
	...
	public SqlServerInboxStoreLeaseFencingShould(SqlServerInboxStoreContainerFixture fixture) =>
		_fixture = fixture;
	...
	/// </summary>
	private async Task<(SqlServerInboxStore Store, string MessageId, string HandlerType, LeaseToken TermA, LeaseToken TermB)>
		LapsedReclaimAsync()
	{
	...
	// lost or gained in the round trip between the lease column and the encoded term.
	[Fact]
	public async Task CompleteUnderALiveTerm()
	{
		...
		(await store.CompleteAsync(messageId, handlerType, term, ct).ConfigureAwait(false)).ShouldBeTrue(
			...
			+ "the round-trip through the lease column and every finalization fails closed");
	...
	//
	// Reclaim admits only when the recorded expiry is STRICTLY earlier than SYSUTCDATETIME(), and the
	// replacement is that same clock plus a non-negative duration. Relaxing that one comparison to <= would
	// let a reclaim reissue the same term and the fence would stop discriminating without failing.
	[Fact]
	public async Task IssueAStrictlyGreaterTermToTheReclaimingCaller()
	{
		...
		var a = DateTimeOffset.ParseExact(termA.Value, "O", CultureInfo.InvariantCulture, DateTimeStyles.None);
		var b = DateTimeOffset.ParseExact(termB.Value, "O", CultureInfo.InvariantCulture, DateTimeStyles.None);
	...
	private async Task<SqlServerInboxStore> CreateStoreAsync()
	{
		_fixture.DockerAvailable.ShouldBeTrue(
			"SQL Server container must be available — the real-infra fencing lock is never skipped.");
	...
	private async Task EnsureTableAsync()
	{
		...
		// Mirrors the shipped SQL Server inbox schema. LeaseExpiresAtUtc is the term column — no schema
		// change was needed for fencing, because the store already wrote a server-side expiry here.
		// Create-if-absent, never DROP: every arm keys on a fresh GUID so rows cannot collide, and a
		// surviving table cannot make an arm pass that would otherwise fail.
		var sql = $"""
