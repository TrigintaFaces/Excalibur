...
/// <summary>
/// Binds <see cref="ElasticsearchInboxStore"/> to the SHIPPED
...
[Trait("Pattern", "STORE")]
public sealed class ElasticsearchInboxStoreKitConformanceShould : InboxStoreConformanceTestKit, IAsyncLifetime
{
	private readonly ElasticsearchInboxStoreContainerFixture _fixture;
	private IInboxStore? _store;

	/// <summary>
	/// Initializes a new instance of the <see cref="ElasticsearchInboxStoreKitConformanceShould"/> class.
	/// </summary>
	/// <param name="fixture">The Elasticsearch container fixture, shared by the collection.</param>
	public ElasticsearchInboxStoreKitConformanceShould(ElasticsearchInboxStoreContainerFixture fixture)
	{
		_fixture = fixture;
	}
	...
	protected override IInboxStore CreateStore()
	{
		...
		// tenant around one store instead of addressing one partition for every arm.
		_store = new ElasticsearchInboxStore(
			_fixture.Client,
			options,
	...
	/// <inheritdoc/>
	/// <remarks>
	/// Deletes the index, which the store recreates on its next write. Data-only with respect to the
	/// store handle, which the kit requires: it runs this between construction and each arm.
	/// </remarks>
	protected override Task CleanupAsync() => _fixture.DeleteIndexAsync();
