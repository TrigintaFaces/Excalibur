...
/// <summary>
/// Elasticsearch container fixture for the Elasticsearch <c>IInboxStore</c> real-infrastructure conformance.
/// </summary>
/// <remarks>
/// Mirrors the Elasticsearch outbox-store fixture. Extends <see cref="ContainerFixtureBase"/>: Docker is
/// required, so a missing container surfaces as a failure rather than a silent pass. Security/TLS are
/// disabled on the container, and the exposed <see cref="Client"/> is built with the SDK's DEFAULT
/// serializer settings — no custom converter — so the store talks to real infrastructure exactly as a
/// consumer's default client would. A unique index name per run isolates the suite, and
/// <see cref="DeleteIndexAsync"/> deletes the index between tests.
/// </remarks>
public sealed class ElasticsearchInboxStoreContainerFixture : ContainerFixtureBase
{
	...
	protected override async Task InitializeContainerAsync(CancellationToken cancellationToken)
	{
		_container = new ElasticsearchBuilder()
			...
			.WithName($"es-inbox-test-{Guid.NewGuid():N}")
	...
	/// <summary>
	/// Deletes the fixture's inbox index between tests (best effort).
	/// </summary>
