...
/// <summary>
/// Binds <see cref="SqlServerInboxStore"/> to the SHIPPED <see cref="InboxStoreConformanceTestKit"/>
/// against a live SQL Server container.
/// </summary>
/// <remarks>
/// This suite derives the kit a consumer can obtain from the published Excalibur.Testing.Conformance
/// package, rather than the private in-repo base. The kit carries tenant-isolation arms that vary the
/// AMBIENT tenant around one store; those arms have never run against this backend.
/// </remarks>
...
[Trait("Pattern", "STORE")]
public sealed class SqlServerInboxStoreKitConformanceShould : InboxStoreConformanceTestKit, IAsyncLifetime
{
	private readonly SqlServerInboxStoreContainerFixture _fixture;
	private IInboxStore? _store;

	/// <summary>
	/// Initializes a new instance of the <see cref="SqlServerInboxStoreKitConformanceShould"/> class.
	/// </summary>
	/// <param name="fixture">The SQL Server container fixture, shared by the collection.</param>
	public SqlServerInboxStoreKitConformanceShould(SqlServerInboxStoreContainerFixture fixture)
	{
		_fixture = fixture;
	}
	...
	/// awaited. <see cref="CreateStoreAsync"/> is the override this suite uses.
	/// </remarks>
	protected override IInboxStore CreateStore() =>
		throw new NotSupportedException(
			$"{nameof(SqlServerInboxStoreKitConformanceShould)} overrides {nameof(CreateStoreAsync)}; "
	...
	protected override async Task<IInboxStore> CreateStoreAsync()
	{
		...
		// RequireTenant = true is REQUIRED and is read from the options, not inferred from the presence of
		// a context: SqlServerInboxStore.cs:136 computes the mode as
		// `tenantContextOptions?.Value.RequireTenant ?? false`. The fixture's DDL
		// (SqlServerInboxStoreContainerFixture.cs:97-99) declares TenantId NOT NULL inside
		// PRIMARY KEY CLUSTERED (MessageId, HandlerType, TenantId), so a single-tenant store is refused by
		// the schema contract at SqlServerInboxStore.cs:200.
		var tenancy = Options.Create(new TenantContextOptions { RequireTenant = true });

		// The AMBIENT context: the kit's isolation arms use ONE store and vary the ambient tenant around
		// it. A fixed context would address one partition for every arm and pass isolation vacuously.
	...
	/// <inheritdoc/>
	/// <remarks>
	/// Data-only (TRUNCATE), never disposal. The kit runs this before each arm, so a cleanup that disposed
	/// the store would hand every arm a dead handle.
	/// </remarks>
