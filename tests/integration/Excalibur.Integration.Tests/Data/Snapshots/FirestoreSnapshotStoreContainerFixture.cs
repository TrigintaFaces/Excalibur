...
/// <summary>
/// Firestore-emulator container fixture for the Firestore SnapshotStore real-infrastructure conformance.
/// </summary>
/// <remarks>
/// Mirrors the Firestore saga-store fixture's emulator setup (explicit endpoint + insecure channel
/// credentials, the reliable way to reach the emulator) so the store binds the emulator-connected
/// <see cref="FirestoreDb"/> built with the SDK's default serializer settings. Extends
/// <see cref="ContainerFixtureBase"/>: Docker is required, so a missing emulator surfaces as a failure
/// rather than a silent pass. A unique collection name per run isolates the suite, and
/// <see cref="CleanupCollectionAsync"/> deletes the collection's documents between tests.
/// </remarks>
public sealed class FirestoreSnapshotStoreContainerFixture : ContainerFixtureBase
{
	...
	/// <summary>
	/// Gets the unique collection name for this fixture's snapshot documents.
	/// </summary>
	public string CollectionName { get; } = $"snapshots_{Guid.NewGuid():N}";

	/// <summary>
	/// Gets the emulator endpoint (also fed to options as EmulatorHost to satisfy Validate()).
	/// </summary>
	...
	protected override async Task InitializeContainerAsync(CancellationToken cancellationToken)
	{
		_container = new FirestoreBuilder()
			...
			.WithName($"firestore-snapshotstore-test-{Guid.NewGuid():N}")
			.WithCleanUp(true)
			.Build();
	...
	/// <summary>
	/// Deletes all documents from the fixture's snapshot collection between tests.
	/// </summary>
