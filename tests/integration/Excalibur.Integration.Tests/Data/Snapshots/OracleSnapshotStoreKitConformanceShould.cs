...
/// <summary>
/// Runs the <strong>shipped</strong> <see cref="SnapshotStoreConformanceTestKit"/> against a real Oracle
/// container, through the same <see cref="OracleSnapshotStore"/> a consumer constructs.
/// </summary>
/// <remarks>
/// See <c>PostgresSnapshotStoreKitConformanceShould</c> for why the internal
/// <c>OracleSnapshotStoreConformanceShould</c> in this directory does not exercise the published
/// contract. This suite closes the same gap for Oracle, reusing the existing
/// <see cref="OracleSnapshotStoreFixture"/> rather than standing up a second container.
/// </remarks>
...
[Trait("Pattern", "STORE")]
public sealed class OracleSnapshotStoreKitConformanceShould : SnapshotStoreConformanceTestKit,
	IClassFixture<OracleSnapshotStoreFixture>, IAsyncLifetime
{
	private readonly OracleSnapshotStoreFixture _fixture;

	/// <summary>Initializes a new instance of the <see cref="OracleSnapshotStoreKitConformanceShould"/> class.</summary>
	/// <param name="fixture">The shared Oracle snapshot-store fixture.</param>
	public OracleSnapshotStoreKitConformanceShould(OracleSnapshotStoreFixture fixture) => _fixture = fixture;

	/// <summary>Confirms the container is up and its schema initialized before any arm resolves the store.</summary>
	public async ValueTask InitializeAsync()
	{
		_fixture.DockerAvailable.ShouldBeTrue(
			"Oracle snapshot conformance runs against real infrastructure and is never skipped: a "
			+ "skipped suite reports the same green as a passing one.");
	...
	/// <c>OracleSnapshotStore(string, ILogger)</c> constructor delegates without one, silently testing
	/// the untenanted path. <see cref="ConformanceAmbientTenantContext"/> is the shipped kit helper. The
	/// schema is the container's resolved connecting user rather than the store's default, which does not
	/// exist in a stock <c>oracle-free</c> image.
	/// </remarks>
	protected override Task<ISnapshotStore> CreateStoreAsync() =>
		Task.FromResult<ISnapshotStore>(new OracleSnapshotStore(
