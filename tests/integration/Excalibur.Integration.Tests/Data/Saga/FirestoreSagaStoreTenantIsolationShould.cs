...
/// <summary>
/// Author≠implementer real-Firestore (emulator) lock for the saga tenant-isolation keystone (bead
/// <c>8rqzdl</c>): the <see cref="FirestoreSagaStore"/> derives a tenant scope from the ambient
...
/// returns <see langword="null"/> (RED against the pre-<c>ec993cbbf</c> shape, which keyed on the saga id
/// alone and returned another tenant's saga). LIVENESS — the owning tenant still loads its own saga.
/// </para>
/// <para>
/// <b>verify-against-real-infra-not-mock:</b> real Firestore emulator (TestContainers). NON-SKIPPED. Both
/// stores share one per-run collection so a cross-tenant read has a real document to (not) match.
/// </para>
/// </remarks>
...
public sealed class FirestoreSagaStoreTenantIsolationShould 
{
	...
	private readonly string _collectionName = "sagas_iso_" + Guid.NewGuid().ToString("N");

	public FirestoreSagaStoreTenantIsolationShould(FirestoreSagaStoreContainerFixture fixture) => _fixture = fixture;
