...
/// <summary>
/// Runs the SHIPPED <see cref="SagaStoreConformanceTestKit"/> against the real SQL Server saga store.
/// </summary>
/// <remarks>
/// <para>
...
public sealed class SqlServerSagaStoreKitConformanceShould : SagaStoreConformanceTestKit, IAsyncLifetime
{
	private readonly SqlServerSagaStoreContainerFixture _fixture;

	public SqlServerSagaStoreKitConformanceShould(SqlServerSagaStoreContainerFixture fixture) => _fixture = fixture;
	...
	protected override void ConfigureProvider(IServiceCollection services) =>
		services.AddSqlServerSagaStore(options =>
		{
	...
	/// <inheritdoc/>
	/// <remarks>
	/// SqlServerSagaStore is version-gated: the save is a MERGE whose update branch requires
	/// <c>target.Version = @ExpectedVersion</c> and whose insert branch is guarded to
	/// <c>@ExpectedVersion = 0</c> (no-resurrect), and a 0-row MERGE is surfaced as
