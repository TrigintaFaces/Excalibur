...
/// <summary>
/// b52l9e (4vlmds) — real-SQL-Server lock for the cross-tenant WRITE VICTIM: a scoped save must never
/// overwrite another tenant's saga row that happens to carry the same saga id. Ports the pattern already
/// proven on Postgres (<see cref="PostgresSagaWriteVictimTenantIsolationShould"/>) to SQL Server.
/// </summary>
/// <remarks>
/// <para>
/// SQL Server's MERGE binds its tenant term unconditionally in the ON clause
/// (<c>SaveSagaRequest{TSagaState}</c>: <c>const string onTenant = " AND target.TenantId = @TenantId";</c>),
/// so the match — and therefore both the UPDATE and INSERT branches — can never address another tenant's
/// row. This lock proves that property against the real MERGE rather than the source comment describing
/// it, and pins it against regression: RED on the mutant that makes <c>onTenant</c> conditional again
/// (the exact shape the fragment's own history comment says it used to be).
/// </para>
/// <para>
...
public sealed class SqlServerSagaWriteVictimTenantIsolationShould(SqlServerSagaStoreContainerFixture fixture)
{
	...
	public async Task NotOverwriteAnotherTenantsSagaSharingTheSameSagaId()
	{
		fixture.DockerAvailable.ShouldBeTrue(
			"SQL Server container must be available — a cross-tenant overwrite destroys saga state in place, "
	...
	private SqlServerSagaStore CreateStore(ITenantContext ambientTenant) =>
		new(
			fixture.ConnectionString,
			Options.Create(new SqlServerSagaStoreOptions
			{
	...
	private static Task SaveAsync(SqlServerSagaStore store, Guid sagaId, string payload, long version = 0) =>
		store.SaveAsync(
