...
public sealed class SecurityAuditMaintenanceBoundedDeleteShould(ElasticsearchContainerFixture fixture)
	: ElasticsearchAuditTestBase(fixture)
{
	...
	// The path under test is date-bound, bounded delete; it does not mint or verify integrity tags here, so a
	// fixed non-empty key satisfies the ctor dependency without affecting the deletion behavior being asserted.
