...
/// <c>IClaimableInboxStore.TryAcquireLeaseAsync(messageId, handlerType, leaseDuration, ct)</c>. The CAS is a Lua
...
public sealed class RedisInboxStoreLeaseReclaimShould
{
	...
	public async Task Reclaim_the_message_after_the_lease_expires()
	{
		...
		// Mark BEFORE the acquiring claim. The lease is stamped at the REDIS SERVER's clock (redis.call('TIME'))
		...
		// meaningful if its observation demonstrably landed INSIDE the window it asserts. Two Redis round trips
