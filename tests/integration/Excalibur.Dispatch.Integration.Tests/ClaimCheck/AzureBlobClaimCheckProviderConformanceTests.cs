...
/// <summary>
/// Runs the shared claim-check conformance kit against the REAL <see cref="AzureBlobClaimCheckProvider"/>
/// on an Azurite container.
/// </summary>
/// <remarks>
/// <para>
/// Reuses the shared <see cref="AzuriteContainerFixture"/> (via <see cref="AzuriteTestCollection"/>)
/// that the Compliance suites already stand up, rather than starting a second Azurite container.
/// </para>
/// <para>
/// Until this class existed, the only type deriving <see cref="ClaimCheckProviderConformanceTestKit"/>
/// was an in-memory dictionary reference implementation. This store instead uploads/downloads through
/// a real <see cref="Azure.Storage.Blobs.BlobContainerClient"/> and translates Azure's own
/// not-found/error shapes into the contract's exceptions.
/// </para>
/// <para>
/// The expiry arms bind the retention contract to this provider. Blob storage has no per-blob
/// time-to-live -- its lifecycle management policies are account-wide and run on a daily schedule -- so
/// <see cref="AzureBlobClaimCheckProvider.RetrieveAsync"/> enforces expiry itself against
/// <see cref="ClaimCheckReference.ExpiresAt"/>, and a zero retention period leaves that unset so the
/// payload never expires. A payload that has expired and one that was never stored raise the same
/// <see cref="KeyNotFoundException"/>, because both are equally unretrievable.
/// </para>
/// </remarks>
...
public sealed class AzureBlobClaimCheckProviderConformanceTests : ClaimCheckProviderConformanceTestKit
{
	private readonly AzuriteContainerFixture _fixture;

	public AzureBlobClaimCheckProviderConformanceTests(AzuriteContainerFixture fixture) => _fixture = fixture;
	...
	private IClaimCheckProvider CreateProvider(TimeSpan ttl)
	{
		_fixture.DockerAvailable.ShouldBeTrue(
			"an Azurite container must be available - real-infra claim-check conformance is never "
			+ "skipped, because an arm that passes by being skipped is indistinguishable from one that "
			+ "passed by working.");

		var options = Microsoft.Extensions.Options.Options.Create(new ClaimCheckOptions
		{
			PayloadThreshold = 256 * 1024,
			DefaultTtl = ttl,
			EnableCompression = false,
			ConnectionString = _fixture.BlobConnectionString,
			ContainerName = $"claimcheck-{Guid.NewGuid():N}"
		});

		return new AzureBlobClaimCheckProvider(options, NullLogger<AzureBlobClaimCheckProvider>.Instance);
	}
