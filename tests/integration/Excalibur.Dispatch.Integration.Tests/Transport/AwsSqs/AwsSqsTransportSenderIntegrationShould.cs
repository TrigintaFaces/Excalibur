...
public sealed class AwsSqsTransportSenderIntegrationShould
{
	private readonly AwsSqsContainerFixture _fixture;

	public AwsSqsTransportSenderIntegrationShould(AwsSqsContainerFixture fixture)
	{
		_fixture = fixture;
	}
	...
	public async Task SendMessage_DeliversToQueue()
	{
		...
		// Act
		var sendResponse = await _fixture.SqsClient.SendMessageAsync(new SendMessageRequest
		{
			QueueUrl = queueUrl,
	...
	public async Task SendBatchMessages_AllDeliverSuccessfully()
	{
		...
		// Act
		var batchResponse = await _fixture.SqsClient.SendMessageBatchAsync(new SendMessageBatchRequest
		{
			QueueUrl = queueUrl,
			Entries = entries,
		}).ConfigureAwait(false);
	...
	public async Task SendMessageToFifoQueue_OrderPreserved()
	{
		...
		for (var attempt = 0; attempt < 5 && receivedBodies.Count < messageCount; attempt++)
		{
			...
			foreach (var msg in receiveResponse.Messages)
			{
				...
				await _fixture.SqsClient.DeleteMessageAsync(queueUrl, msg.ReceiptHandle).ConfigureAwait(false);
			}
		}
	...
	public async Task QueueUrl_IsAccessible()
	{
		...
		// Act
		var createResponse = await _fixture.SqsClient.CreateQueueAsync(new CreateQueueRequest
		{
			QueueName = queueName,
		}).ConfigureAwait(false);

		// Assert — GetQueueUrl should resolve
		var getUrlResponse = await _fixture.SqsClient.GetQueueUrlAsync(queueName).ConfigureAwait(false);
