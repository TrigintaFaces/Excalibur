...
/// <summary>
/// Runs the shipped dead-letter conformance kit against the REAL SQL Server dead-letter store.
/// </summary>
/// <remarks>
/// <para>
/// Until this class and its Postgres twin existed, every type deriving
/// <see cref="DeadLetterStoreConformanceTestKit"/> bound an in-memory store or a decorator over one, so
/// the whole kit ran against implementations with no SQL in them. <c>SqlServerDeadLetterStore</c> had an
/// integration test of its own, but nothing held it to the contract the shipped kit publishes.
/// </para>
/// <para>
...
public sealed class SqlServerDeadLetterStoreConformanceTests : DeadLetterStoreConformanceTestKit, IAsyncLifetime
{
	...
	/// <summary>Initializes a new instance of the <see cref="SqlServerDeadLetterStoreConformanceTests"/> class.</summary>
	/// <param name="fixture">The shared SQL Server container.</param>
	public SqlServerDeadLetterStoreConformanceTests(SqlServerFixture fixture) => _fixture = fixture;

	/// <inheritdoc />
	public async ValueTask InitializeAsync() =>
		await ShippedDeadLetterSchema.ProvisionSqlServerAsync(_fixture.ConnectionString, TestContext.Current.CancellationToken)
	...
	protected override IDeadLetterStore CreateStore(ITenantContext ambientTenant) =>
		new SqlServerDeadLetterStore(
			Microsoft.Extensions.Options.Options.Create(
				new SqlServerDeadLetterOptions { ConnectionString = _fixture.ConnectionString }),
			ambientTenant,
			EnabledTestLogger.Create<SqlServerDeadLetterStore>());
	...
	protected override Task CleanupAsync() =>
		ShippedDeadLetterSchema.TruncateSqlServerAsync(_fixture.ConnectionString, TestContext.Current.CancellationToken);
