...
/// <summary>
/// Integration tests for MongoDbEventStore OpenTelemetry instrumentation.
...
public sealed class MongoDbEventStoreTelemetryShould : IClassFixture<MongoDbEventStoreTelemetryTestFixture>, IAsyncLifetime
{
	...
	public MongoDbEventStoreTelemetryShould(MongoDbEventStoreTelemetryTestFixture fixture)
	{
		_fixture = fixture;
	}
	...
	[Fact]
	public async Task SetConcurrencyConflictOnMongoDuplicateKeyError()
	{
		// Arrange
		// This tests the case where MongoDB returns duplicate key error (11000)
		// which can happen in race conditions via MongoBulkWriteException or MongoWriteException
		_fixture.ClearRecordedActivities();
		var eventStore = _fixture.CreateEventStore();
		var aggregateId = Guid.NewGuid().ToString();
		...
		var result = await eventStore.AppendAsync(aggregateId, "TestAggregate", secondEvents, -1, CancellationToken.None)
			.ConfigureAwait(false);
	...
	private static MongoDbTelemetryTestDomainEvent CreateTestEvent()
	{
		return new MongoDbTelemetryTestDomainEvent
		{
