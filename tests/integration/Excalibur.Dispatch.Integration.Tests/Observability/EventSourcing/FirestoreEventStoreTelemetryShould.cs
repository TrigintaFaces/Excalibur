...
/// <summary>
/// Integration tests for FirestoreEventStore OpenTelemetry instrumentation.
/// Validates that ActivitySource spans are correctly created with proper tags
/// when executing actual database operations against the Firestore emulator.
/// </summary>
/// <remarks>
/// Note: The Firestore emulator may have limitations in some CI environments.
...
{
	private readonly FirestoreEventStoreTelemetryTestFixture _fixture;

	public FirestoreEventStoreTelemetryShould(FirestoreEventStoreTelemetryTestFixture fixture)
	{
		_fixture = fixture;
	}
	...
	[Fact]
	public async Task SetConcurrencyConflictOnFirestoreVersionMismatch()
	{
		// Arrange
		// This tests the case where Firestore detects a version mismatch during transaction
		// Firestore uses a single exception type (RpcException with StatusCode.AlreadyExists)
		// which is simpler than DynamoDB's dual-exception pattern
		_fixture.ClearRecordedActivities();
		var eventStore = _fixture.CreateEventStore();
		var aggregateId = Guid.NewGuid().ToString();
	...
	private static FirestoreTelemetryTestDomainEvent CreateTestEvent()
	{
		return new FirestoreTelemetryTestDomainEvent
		{
