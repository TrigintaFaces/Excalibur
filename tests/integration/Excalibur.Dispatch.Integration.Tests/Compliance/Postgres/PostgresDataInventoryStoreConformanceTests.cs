...
/// <summary>
/// Runs the published data-inventory conformance kit against the PostgreSQL store, on a real server.
/// </summary>
/// <remarks>
/// <para>
...
/// present in BOTH durable providers survived a kit written to catch it.
/// </para>
/// <para>
/// <b>Why the SQL Server sibling is not enough on its own.</b> The two dialects do not agree by default
/// on the comparison this contract rests on: SQL Server's database default collation is typically
/// case-INSENSITIVE while PostgreSQL's is deterministic. An isolation guarantee proved on one engine is
/// therefore not evidence for the other, and holding both to the same kit is what turns "it works here"
...
/// perturbed by residue another arm left behind.
/// </para>
/// <para>
...
public sealed class PostgresDataInventoryStoreConformanceTests : DataInventoryStoreConformanceTestKit
{
	...
	public PostgresDataInventoryStoreConformanceTests(PostgresFixture fixture) => _fixture = fixture;

	/// <inheritdoc />
	protected override IDataInventoryStore CreateStore()
	{
		var options = new PostgresDataInventoryStoreOptions
		{
			...
			RegistrationsTableName = $"data_inventory_registrations_{_suffix}",
			DiscoveredLocationsTableName = $"discovered_data_locations_{_suffix}",
		...
		return new PostgresDataInventoryStore(
