...
/// <summary>
/// Binds tenant isolation on the Postgres legal-hold store — the control that BLOCKS erasure, which makes
/// its failure modes asymmetric in a way the erasure store's are not. The Postgres half of p2kthq: SQL
/// Server carried this exact lock (<c>SqlServerLegalHoldStoreTenantIsolationShould</c>) and Postgres carried
/// none — the shared conformance kit exercises the tenant-filter <em>parameter</em>, but its own remarks
/// state plainly that whether that filter detects a cross-tenant disclosure "is not asserted here".
/// </summary>
/// <remarks>
/// <para>
/// <b>Why this file runs against a real server.</b> The defect this mirrors is in a SQL predicate: the
/// reads branched on a caller-supplied nullable tenant, and one of them built two entire statements — the
/// one without a tenant argument carried no tenant term at all. An in-memory store filters in C# and never
/// executes either.
/// </para>
/// <para>
...
public sealed class PostgresLegalHoldStoreTenantIsolationShould : IntegrationTestBase
{
	private const string OwningTenant = "pg-hold-tenant-owning";
	private const string ForeignTenant = "pg-hold-tenant-foreign-owns-nothing";

	private readonly PostgresFixture _fixture;
	private readonly string _suffix = Guid.NewGuid().ToString("N")[..8];

	public PostgresLegalHoldStoreTenantIsolationShould(PostgresFixture fixture) => _fixture = fixture;
	...
	/// it fails OPEN on a control whose entire purpose is to refuse.
	/// </remarks>
	[Fact]
	...
	private PostgresLegalHoldStore Build(ITenantContext tenantContext, bool requireTenant) => new(
		Microsoft.Extensions.Options.Options.Create(new PostgresLegalHoldStoreOptions
		{
			...
			TableName = $"legal_holds_tenant_isolation_{_suffix}",
