...
/// <summary>
/// Binds tenant isolation on the Postgres erasure-request store — the record of who asked to be erased,
/// when, on what legal basis, and whether it happened. The Postgres half of p2kthq: SQL Server carried this
/// exact lock (<c>SqlServerErasureStoreTenantIsolationShould</c>) and Postgres carried none — the shared
/// conformance kit exercises the tenant-filter <em>parameter</em>, but its own remarks state plainly that
/// whether that filter detects a cross-tenant disclosure "is not asserted here".
/// </summary>
/// <remarks>
/// <para>
/// <b>Why this file runs against a real server.</b> The defect this mirrors is in a SQL predicate: the
/// reads branched on a caller-supplied nullable tenant, so a caller who passed nothing got no predicate at
/// all. An in-memory store filters in C# and never executes that statement, so a unit suite over it can
/// pass with a perfect score and remain structurally incapable of observing the fault.
/// </para>
/// <para>
...
/// assert that a particular parameter is bound or a particular column compared.
/// </para>
/// <para>
...
public sealed class PostgresErasureStoreTenantIsolationShould : IntegrationTestBase
{
	private const string OwningTenant = "pg-erasure-tenant-owning";
	private const string ForeignTenant = "pg-erasure-tenant-foreign-owns-nothing";

	private readonly PostgresFixture _fixture;
	private readonly string _suffix = Guid.NewGuid().ToString("N")[..8];

	public PostgresErasureStoreTenantIsolationShould(PostgresFixture fixture) => _fixture = fixture;
	...
	private PostgresErasureStore CreateStore(string ambientTenant) => new(
		Microsoft.Extensions.Options.Options.Create(new PostgresErasureStoreOptions
		{
			ConnectionString = _fixture.ConnectionString,
			SchemaName = "compliance",

			// Lower-case deliberately: Postgres folds unquoted identifiers, matching the shipped store's
			// own convention. Suffixed per instance so this file's tables never collide with a sibling
			// arm's on the shared container.
			RequestsTableName = $"erasure_requests_tenant_isolation_{_suffix}",
			CertificatesTableName = $"erasure_certificates_tenant_isolation_{_suffix}",
