...
public sealed class RateLimitingMiddlewareLimiterLifetimeShould
{
	...
	public async Task KeepTheLimiterAndTheSpentBudgetOfAnActivelyLimitedKey()
	{
		...
		do
		{
			...
			// The elapsed duration IS the semantic under test -- a key kept active for longer than the
			// reclaim window must never regain budget -- so there is no condition to poll for instead.
			await Task.Delay(TimeSpan.FromMilliseconds(500), CancellationToken.None); // delay-ok: paces requests across the real reclaim window under test
		}
