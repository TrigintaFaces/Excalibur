...
/// <summary>
/// Unit tests for <see cref="CosmosDbInboxBuilder"/> — 5 connection overloads,
...
public sealed class CosmosDbInboxBuilderShould : UnitTestBase
{
	private const string TestConnectionString =
		"AccountEndpoint=https://localhost:8081/;AccountKey=dGVzdA==";

	private static (CosmosDbInboxBuilder Builder, CosmosDbInboxOptions Options) CreateBuilder()
	{
		var options = new CosmosDbInboxOptions();
		var builder = new CosmosDbInboxBuilder(options);
	...
	public void BindConfiguration_StorePathOnBuilder()
	{
		...
		builder.BindConfiguration("CosmosDb:Inbox");

		builder.BindConfigurationPath.ShouldBe("CosmosDb:Inbox");
	}
	...
	public void ContainerName_SetValueOnOptions()
	{
		...
		builder.ContainerName("my-inbox");

		options.ContainerName.ShouldBe("my-inbox");
	}
	...
	{
		var (builder, _) = CreateBuilder();
		var result = builder.BindConfiguration("CosmosDb:Inbox");
		result.ShouldBeSameAs(builder);
	}
