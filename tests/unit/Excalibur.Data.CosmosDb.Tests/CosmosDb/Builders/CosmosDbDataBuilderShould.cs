...
/// <summary>
/// Unit tests for <see cref="CosmosDbDataBuilder"/> — 5 connection overloads,
...
public sealed class CosmosDbDataBuilderShould : UnitTestBase
{
	private const string TestConnectionString =
		"AccountEndpoint=https://localhost:8081/;AccountKey=dGVzdA==";

	private static (CosmosDbDataBuilder Builder, CosmosDbOptions Options) CreateBuilder()
	{
		var options = new CosmosDbOptions();
		var builder = new CosmosDbDataBuilder(options);
	...
	public void BindConfiguration_StorePathOnBuilder()
	{
		...
		builder.BindConfiguration("CosmosDb:Data");

		builder.BindConfigurationPath.ShouldBe("CosmosDb:Data");
	}
	...
	public void ContainerName_SetValueOnOptions()
	{
		...
		options.DefaultContainerName.ShouldBe("my-container");
	}
	...
	{
		var (builder, _) = CreateBuilder();
		var result = builder.BindConfiguration("CosmosDb:Data");
		result.ShouldBeSameAs(builder);
	}
