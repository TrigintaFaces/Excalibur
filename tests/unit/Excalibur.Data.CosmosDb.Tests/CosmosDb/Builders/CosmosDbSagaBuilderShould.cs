...
/// <summary>
/// Unit tests for <see cref="CosmosDbSagaBuilder"/> — 5 connection overloads,
...
public sealed class CosmosDbSagaBuilderShould : UnitTestBase
{
	private const string TestConnectionString =
		"AccountEndpoint=https://localhost:8081/;AccountKey=dGVzdA==";

	private static (CosmosDbSagaBuilder Builder, CosmosDbSagaOptions Options) CreateBuilder()
	{
		var options = new CosmosDbSagaOptions();
		var builder = new CosmosDbSagaBuilder(options);
	...
	public void BindConfiguration_StorePathOnBuilder()
	{
		...
		builder.BindConfiguration("CosmosDb:Saga");

		builder.BindConfigurationPath.ShouldBe("CosmosDb:Saga");
	}
	...
	public void ContainerName_SetValueOnOptions()
	{
		...
		builder.ContainerName("my-sagas");
	...
	{
		var (builder, _) = CreateBuilder();
		var result = builder.BindConfiguration("CosmosDb:Saga");
		result.ShouldBeSameAs(builder);
	}
