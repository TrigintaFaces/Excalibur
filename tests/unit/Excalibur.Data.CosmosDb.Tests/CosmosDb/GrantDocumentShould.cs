...
/// <summary>
/// Unit tests for the GrantDocument class.
/// </summary>
/// <remarks>
/// Sprint 514 (S514.3): CosmosDB unit tests.
/// Tests verify document properties and factory methods.
/// Note: GrantDocument is internal, so we use reflection to test it.
/// </remarks>
[Trait("Category", TestCategories.Unit)]
[Trait("Component", "CosmosDb")]
[Trait("Feature", "Authorization")]
public sealed class GrantDocumentShould
{
	private readonly Type _documentType;

	public GrantDocumentShould()
	{
		// Get the internal type via reflection
		var assembly = typeof(CosmosDbAuthorizationOptions).Assembly;
		_documentType = assembly.GetType("Excalibur.Data.CosmosDb.Authorization.GrantDocument")!;
	}
	...
	public void CreateId_ReturnsCompositeId()
	{
		...
		// Act
		var result = (string)createIdMethod!.Invoke(null, new object?[] { "user-1", "tenant-1", "role", "admin" })!;

		// Assert
		result.ShouldBe("user-1:tenant-1:role:admin");
	}
	...
	public void CreateId_HandlesNullTenant()
	{
		...
		// Act
		var result = (string)createIdMethod!.Invoke(null, new object?[] { "user-1", null, "role", "admin" })!;

		// Assert
		result.ShouldBe("user-1:null:role:admin");
	}
	...
	public void IsRevoked_DefaultsToFalse()
	{
		// Arrange & Act
		var document = Activator.CreateInstance(_documentType);
		var property = _documentType.GetProperty("IsRevoked");

		// Assert
		property.GetValue(document).ShouldBe(false);
	}

	[Fact]
	public void RevokedBy_DefaultsToNull()
	{
		// Arrange & Act
		var document = Activator.CreateInstance(_documentType);
		var property = _documentType.GetProperty("RevokedBy");
	...
	[Fact]
	public void RevokedOn_DefaultsToNull()
	{
		// Arrange & Act
		var document = Activator.CreateInstance(_documentType);
		var property = _documentType.GetProperty("RevokedOn");
	...
	[Fact]
	public void IsRevoked_HasCorrectJsonPropertyName()
	{
		// Arrange
		var property = _documentType.GetProperty("IsRevoked");
		var attribute = property.GetCustomAttribute<JsonPropertyNameAttribute>();
		...
		// Assert
		attribute.ShouldNotBeNull();
		attribute.Name.ShouldBe("is_revoked");
	}
