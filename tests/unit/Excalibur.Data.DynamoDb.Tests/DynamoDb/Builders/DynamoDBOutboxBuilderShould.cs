...
/// <summary>
/// Unit tests for <see cref="DynamoDBOutboxBuilder"/> — 5 connection overloads,
...
public sealed class DynamoDBOutboxBuilderShould : UnitTestBase
{
    private static DynamoDBOutboxBuilder CreateBuilder() => new();
    ...
    {
        var builder = CreateBuilder();
        builder.BindConfiguration("Outbox:DynamoDB");
        builder.BindConfigurationPath.ShouldBe("Outbox:DynamoDB");
    }
    ...
    {
        var builder = CreateBuilder();
        builder.TableName("outbox-messages");
        builder.TableNameValue.ShouldBe("outbox-messages");
    }
    ...
    {
        var builder = CreateBuilder();
        builder.TablePrefix("qa-");
        builder.TablePrefixValue.ShouldBe("qa-");
    }
    ...
    public void Region_ClearAllOtherConnectionOverloads()
    {
        ...
        builder.Region(RegionEndpoint.SAEast1);
    ...
    public void ClientFactory_ClearAllOtherConnectionOverloads()
    {
        var builder = CreateBuilder();
        Func<IServiceProvider, IAmazonDynamoDB> factory = _ => A.Fake<IAmazonDynamoDB>();
        builder.Region(RegionEndpoint.USWest2);
        builder.ClientFactory(factory);
