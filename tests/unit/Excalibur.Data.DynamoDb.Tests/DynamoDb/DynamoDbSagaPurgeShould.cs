...
/// <summary>
/// Unit locks for <see cref="DynamoDbSagaStore.PurgeCompletedBeforeAsync"/> (bead vd50j3).
/// </summary>
/// <remarks>
/// <para>
...
/// (<c>completedAt</c> present) AND aged (<c>completedAt &lt; threshold</c>), NEVER a running saga
/// (<c>completedAt</c> absent), and returns the deleted count.
/// </para>
/// <para>
/// NON-VACUITY: the running-saga exclusion is the load-bearing guard, enforced server-side by the
/// <c>attribute_exists(#completedAt)</c> predicate in the Scan <c>FilterExpression</c>. A mutant that
/// drops <c>attribute_exists</c> (letting running sagas match) or that removes the <c>#c &lt; :cutoff</c>
/// bound (purging not-yet-aged completed sagas) changes the captured request and turns the filter
...
public sealed class DynamoDbSagaPurgeShould
{
	...
	private static DynamoDbSagaStore CreateStore(IAmazonDynamoDB client)
	{
		var options = Options.Create(new DynamoDbSagaOptions { TableName = TableName });
		return new DynamoDbSagaStore(client, options, A.Fake<ILogger<DynamoDbSagaStore>>(), new DispatchJsonSerializer());
	}
	...
	public async Task Purge_FiltersOnAttributeExistsAndAgeBound_ExcludingRunningSagas()
	{
		...
		// Assert — the load-bearing running-saga guard is the attribute_exists predicate + age bound.
		removed.ShouldBe(0);
