...
/// <summary>
/// Test-only helper (qa71t5 F-5 sweep) that builds the real keyed-MAC <see cref="IAuditIntegrityStrategy"/>
/// via the public DI path (<c>AddAuditIntegrity()</c> + a test signing key) — internal-first (no public
/// production test-factory), drives the genuine strategy (<c>verify-against-real-infra-not-mock</c>).
/// </summary>
internal static class AuditIntegrityTestStrategy
{
    ...
    public static IAuditIntegrityStrategy Create()
    {
        ...
        // SigningKey is init-only → object-initializer via Options.Create (Configure(Action) can't set init).
