...
public sealed class InMemoryPersistenceProviderConformanceTests : PersistenceProviderConformanceTestKit
{
	...
	// The in-memory provider is not an ISqlPersistenceProvider, so both arms below return without
	// asserting. They are wired anyway, and deliberately: the kit's completeness gate requires every arm
	// to be REACHABLE from every deriver, so that a suite whose provider later gains the SQL surface
	// starts exercising them instead of silently continuing to skip what it never declared.
