...
/// <summary>
/// Unit tests for <see cref="QuartzJobOptions"/>.
/// </summary>
[Trait("Category", "Unit")]
...
public sealed class JobConfigurationShould
{
	...
	{
		// Arrange & Act
		var config = new QuartzJobOptions("my-job", "0 * * * * ?");
	...
	{
		// Act
		var config = new QuartzJobOptions("job", "0 0 * * * ?");
	...
	{
		// Act & Assert
		Should.Throw<ArgumentNullException>(() => new QuartzJobOptions(null!, "0 0 * * * ?"));
	}
	...
	{
		// Act & Assert
		Should.Throw<ArgumentNullException>(() => new QuartzJobOptions("job", null!));
	}
	...
	public void SetAllPropertiesAtOnce()
	{
		...
		// Act
		var config = new QuartzJobOptions("my-job-key", "0 30 8 * * ?")
		{
			JobData = data,
