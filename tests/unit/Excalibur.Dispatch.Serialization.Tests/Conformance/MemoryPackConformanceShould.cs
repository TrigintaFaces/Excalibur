...
/// <summary>
/// Sprint 623 A.1: Serializer conformance tests for <see cref="MemoryPackSerializer"/>.
/// </summary>
...
public sealed class MemoryPackConformanceShould : SerializerConformanceTestKit
{
	protected override ISerializer CreateSerializer() => new MemoryPackSerializer();

	protected override object CreateTestObject() => new MemPackConformanceDto
	{
	...
	protected override object CreateEmptyTestObject() => new MemPackConformanceDto();

	protected override Type TestObjectType => typeof(MemPackConformanceDto);

	protected override void AssertObjectsEqual(object expected, object actual)
	{
		var e = (MemPackConformanceDto)expected;
		var a = (MemPackConformanceDto)actual;
	...
	protected override void SerializeTyped(ISerializer serializer, object value, System.Buffers.IBufferWriter<byte> bufferWriter)
		=> serializer.Serialize((MemPackConformanceDto)value, bufferWriter);

	protected override object CreateLargeTestObject()
	{
		var largeString = new string('x', 500_000);
		return new MemPackConformanceDto
		{
			Name = largeString,
			Value = int.MaxValue,
			IsActive = true,
