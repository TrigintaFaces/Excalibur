...
public sealed class TelemetryEventStoreShould : IDisposable
{
	...
	private readonly ConcurrentBag<(string Name, long Value, KeyValuePair<string, object?>[] Tags)> _counterMeasurements = [];
	private readonly ConcurrentBag<(string Name, double Value, KeyValuePair<string, object?>[] Tags)> _histogramMeasurements = [];
