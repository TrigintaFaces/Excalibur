...
/// <summary>
/// CDC Anti-Corruption Layer - Message Translation workflow tests.
...
/// </para>
/// <para>
/// Anti-Corruption Layer Pattern (CDC → Command → Dispatch):
/// CDC Event → Translator → Command → Dispatch → Handler → Domain Event
/// </para>
/// <para>
/// This differs from Sprint 182 tests which tested direct CDC → Domain Event transformation.
/// Sprint 196 introduces an intermediate Command layer to protect the domain model.
/// </para>
/// </remarks>
...
[Trait("Category", "Unit")]
public sealed class CdcMessageTranslationAclWorkflowShould
{
	/// <summary>
	...
	/// </summary>
	[Fact]
	public async Task TranslateCdcInsertToCreateCommand()
	{
		// Arrange
	...
	/// </summary>
	[Fact]
	public async Task TranslateCdcUpdateToUpdateCommand()
	{
		// Arrange
		var executionLog = new ExecutionLog();
		var translator = new CdcToCommandTranslator(executionLog);
	...
	/// </summary>
	[Fact]
	public async Task TranslateCdcDeleteToDeleteCommand()
	{
		// Arrange
		var executionLog = new ExecutionLog();
		var translator = new CdcToCommandTranslator(executionLog);
	...
	/// </summary>
	[Fact]
	public async Task TranslateBatchChangesInOrder()
	{
		// Arrange
		var executionLog = new ExecutionLog();
		var translator = new CdcToCommandTranslator(executionLog);
	...
	/// </summary>
	[Fact]
	public async Task ValidateDataIntegrityBeforeTranslation()
	{
		// Arrange
		var executionLog = new ExecutionLog();
		var translator = new CdcToCommandTranslator(executionLog);
	...
	/// </summary>
	internal sealed class CdcAntiCorruptionPipeline
	{
	...
	/// </summary>
	internal sealed class CdcToCommandTranslator
	{
		private readonly ExecutionLog _log;
	...
	/// </summary>
	internal sealed class SimulatedDispatcher
	{
		private readonly ExecutionLog _log;
	...
	internal sealed class UpdateOrderCommand
	{
		public string OrderId { get; init; } = string.Empty;
	...
	internal sealed class DeleteOrderCommand
	{
		public string OrderId { get; init; } = string.Empty;
	...
	internal sealed class OrderCreatedDomainEvent
	{
		public string OrderId { get; init; } = string.Empty;
		public string CustomerId { get; init; } = string.Empty;
		public decimal TotalAmount { get; init; }
	...
	internal sealed class OrderUpdatedDomainEvent
	{
		public string OrderId { get; init; } = string.Empty;
		public decimal OldTotalAmount { get; init; }
		public decimal NewTotalAmount { get; init; }
	...
	internal sealed class OrderDeletedDomainEvent
	{
		public string OrderId { get; init; } = string.Empty;
		public string Reason { get; init; } = string.Empty;
	}
