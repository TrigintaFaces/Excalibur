...
/// <summary>
/// Binds the document-id composition of <see cref="FirestoreInboxStore"/>.
/// </summary>
/// <remarks>
/// <para>
...
public sealed class FirestoreInboxDocumentIdShould
{
		...
		string rightTenant, string rightMessage, string rightHandler)
	{
		...
		var left = FirestoreInboxStore.ComposeDocumentId(leftTenant, leftMessage, leftHandler);
		var right = FirestoreInboxStore.ComposeDocumentId(rightTenant, rightMessage, rightHandler);
	...
	{
		var id = FirestoreInboxStore.ComposeDocumentId(tenantId, messageId, handlerType);

		id.ShouldStartWith("inbox:");

		var segments = id["inbox:".Length..].Split(':');
	...
	{
		var first = FirestoreInboxStore.ComposeDocumentId(tenantId, messageId, handlerType);
		var second = FirestoreInboxStore.ComposeDocumentId(tenantId, messageId, handlerType);
	...
	{
		var id = FirestoreInboxStore.ComposeDocumentId("tenant-1", "9f8c3d7e-1a2b", "MyApp.Handlers.OrderHandler");

		id.ShouldBe("inbox:tenant-1:9f8c3d7e-1a2b:MyApp.Handlers.OrderHandler");
	}

	/// <summary>
	/// Firestore reserves document ids matching <c>__.*__</c> and rejects a write that uses one. The
	/// reserved untenanted tenant term is <c>__untenanted__</c>, so without a constant prefix every id in
	/// a deployment that does not use multi-tenancy starts with <c>__</c>, and a handler type name ending
	/// in <c>__</c> completes the pattern and fails the write. The first two rows are exactly that case.
	/// </summary>
	[Theory]
	[InlineData("__untenanted__", "a", "H__")]
	[InlineData("__untenanted__", "a__", "__H__")]
	[InlineData("__tenant__", "__m__", "__H__")]
	[InlineData("t", "m", "H")]
	public void NeverComposeAnIdFirestoreReserves(string tenantId, string messageId, string handlerType)
	{
		var id = FirestoreInboxStore.ComposeDocumentId(tenantId, messageId, handlerType);

		// The reserved pattern is anchored at BOTH ends, so an id that cannot begin with "__" cannot
		// match it whatever the terms contain. Asserting the mechanism rather than running the pattern
		// keeps this a statement about why the id is safe, not a coincidence about these four rows.
		id.StartsWith("__", StringComparison.Ordinal)
			.ShouldBeFalse($"Firestore reserves ids matching __.*__, and '{id}' can reach that shape");

		// The other two id rules Firestore enforces, closed by the same encoding.
		id.ShouldNotContain("/");
		id.ShouldNotBe(".");
		id.ShouldNotBe("..");
	}
	...
	public void RejectAnAbsentTerm(string? tenantId, string? messageId, string? handlerType)
		=> Should.Throw<ArgumentNullException>(
			() => FirestoreInboxStore.ComposeDocumentId(tenantId!, messageId!, handlerType!));
	...
	public void RejectATermThatIsNotWellFormedText(int loneSurrogate)
	{
		...
		Should.Throw<ArgumentException>(() => FirestoreInboxStore.ComposeDocumentId(malformed, "m", "H"));
		Should.Throw<ArgumentException>(() => FirestoreInboxStore.ComposeDocumentId("t", malformed, "H"));
		Should.Throw<ArgumentException>(() => FirestoreInboxStore.ComposeDocumentId("t", "m", malformed));
	}

	/// <summary>
	/// SAFETY. Firestore refuses a document id longer than 1500 bytes. Percent-encoding expands, so
	...
	{
		var overLimit = new string('H', 1491);

		var thrown = Should.Throw<ArgumentException>(
			() => FirestoreInboxStore.ComposeDocumentId("t", "m", overLimit));

		thrown.Message.ShouldContain("1500");
	}
	...
	{
		var atLimit = new string('H', 1490);

		var id = FirestoreInboxStore.ComposeDocumentId("t", "m", atLimit);

		Encoding.UTF8.GetByteCount(id).ShouldBe(1500);
	}
}
