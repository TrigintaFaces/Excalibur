...
/// <summary>
/// Unit tests for <see cref="FirestoreDataBuilder"/> — 8 connection/config methods,
/// Firestore-specific last-wins semantics, fluent chaining, and validation guards.
/// </summary>
[Trait(TraitNames.Category, TestCategories.Unit)]
[Trait(TraitNames.Component, TestComponents.Core)]
[Trait("Database", "Firestore")]
public sealed class FirestoreDataBuilderShould : UnitTestBase
{
    private static FirestoreDb CreateTestClient()
        => FirestoreDb.Create("test-project", A.Fake<FirestoreClient>());

    private static (FirestoreDataBuilder Builder, FirestoreOptions Options) CreateBuilder()
    {
        var options = new FirestoreOptions();
        var builder = new FirestoreDataBuilder(options);
    ...
    public void BindConfiguration_StorePathOnBuilder()
    {
        ...
        builder.BindConfiguration("Firestore:Data");

        builder.BindConfigurationPath.ShouldBe("Firestore:Data");
    }
    ...
    public void CollectionName_SetValueOnOptions()
    {
        ...
        builder.CollectionName("my-collection");

        builder.CollectionNameValue.ShouldBe("my-collection");
        options.DefaultCollection.ShouldBe("my-collection");
    }
    ...
    {
        var (builder, _) = CreateBuilder();
        var result = builder.BindConfiguration("Firestore:Data");
        result.ShouldBeSameAs(builder);
    }
    ...
    {
        var (builder, _) = CreateBuilder();
        var result = builder.CollectionName("my-collection");
        result.ShouldBeSameAs(builder);
    }
