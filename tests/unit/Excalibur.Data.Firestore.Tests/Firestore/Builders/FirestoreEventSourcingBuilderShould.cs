...
/// <summary>
/// Unit tests for <see cref="FirestoreEventSourcingBuilder"/> — 8 connection/config methods,
/// Firestore-specific last-wins semantics, fluent chaining, and validation guards.
/// </summary>
[Trait(TraitNames.Category, TestCategories.Unit)]
[Trait(TraitNames.Component, TestComponents.Core)]
[Trait("Database", "Firestore")]
public sealed class FirestoreEventSourcingBuilderShould : UnitTestBase
{
    private static (FirestoreEventSourcingBuilder Builder, FirestoreEventStoreOptions Options) CreateBuilder()
    {
        var options = new FirestoreEventStoreOptions();
        var builder = new FirestoreEventSourcingBuilder(options);
    ...
    public void Constructor_ThrowOnNullOptions()
    {
        Should.Throw<ArgumentNullException>(() =>
            new FirestoreEventSourcingBuilder(null!));
    }
    ...
    public void BindConfiguration_StorePathOnBuilder()
    {
        ...
        builder.BindConfiguration("Firestore:EventSourcing");

        builder.BindConfigurationPath.ShouldBe("Firestore:EventSourcing");
    }
    ...
    public void CollectionName_SetValueOnOptions()
    {
        ...
        builder.CollectionName("my-events");

        builder.CollectionNameValue.ShouldBe("my-events");
        options.EventsCollectionName.ShouldBe("my-events");
    }
    ...
    {
        var (builder, _) = CreateBuilder();
        var result = builder.BindConfiguration("Firestore:EventSourcing");
        result.ShouldBeSameAs(builder);
    }

    [Fact]
    public void CollectionName_ReturnBuilderForChaining()
    {
        var (builder, _) = CreateBuilder();
        var result = builder.CollectionName("my-events");
        result.ShouldBeSameAs(builder);
    }
