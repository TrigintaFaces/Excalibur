...
/// <summary>
/// Execution-semantics tests for <c>MongoDbRetryPolicy</c>.
/// </summary>
/// <remarks>
...
public sealed class MongoDbRetryPolicyExecutionShould : UnitTestBase
{
	[Theory]
	[InlineData(0, 1)]
	[InlineData(1, 2)]
	...
	public async Task InvokeTheRequestOncePlusItsRetryBudget(int maxRetryAttempts, int expectedInvocations)
	{
		...
		var policy = new MongoDbRetryPolicy(maxRetryAttempts, new CapturingLogger(), clock);
		var request = DataRequest.AlwaysThrowing(NewTransientFailure);

		_ = await Should.ThrowAsync<MongoException>(DriveAsync(policy, request, clock));
	...
	public async Task BackOffOnTheExactExponentialScheduleCappedAtMaxDelay()
	{
		...
		var policy = new MongoDbRetryPolicy(5, logger, clock);
		var request = DataRequest.AlwaysThrowing(NewTransientFailure);
	...
	{
		var clock = new FakeTimeProvider();
		var thrown = new List<MongoException>();
		var policy = new MongoDbRetryPolicy(2, new CapturingLogger(), clock);
		var request = DataRequest.AlwaysThrowing(() =>
		{
			var exception = NewTransientFailure();
			thrown.Add(exception);
			return exception;
		});

		var surfaced = await Should.ThrowAsync<MongoException>(DriveAsync(policy, request, clock));
	...
	{
		var clock = new FakeTimeProvider();
		var policy = new MongoDbRetryPolicy(5, new CapturingLogger(), clock);
		var request = DataRequest.ThrowingUntil(2, NewTransientFailure, result: 42);
	...
	public async Task RouteDocumentRequestsThroughTheSameBudgetAndSchedule()
	{
		var clock = new FakeTimeProvider();
		var logger = new CapturingLogger();
		var policy = new MongoDbRetryPolicy(3, logger, clock);
		var request = new ThrowingDocumentRequest(NewTransientFailure);
		...
		_ = await Should.ThrowAsync<MongoException>(PumpAsync(operation, clock));

		request.Invocations.ShouldBe(4);
		logger.RetryDelaysMilliseconds.ShouldBe([2_000d, 4_000d, 8_000d]);
	}

	// MongoConnectionException requires a real ConnectionId - passing default gives it null and its
	// constructor throws ArgumentNullException, so the policy under test is never entered at all. A plain
	// MongoException is what ShouldRetry classifies as transient, and it is constructible without a live
	// server, so it is what these tests drive.
	private static MongoException NewTransientFailure() => new MongoException("transient");

	private static Task<int> DriveAsync(MongoDbRetryPolicy policy, ThrowingDataRequest request, FakeTimeProvider clock) =>
		PumpAsync(
