...
public sealed class MongoDbRetryPolicyShould : UnitTestBase
{
	...
	{
		// MongoDbRetryPolicy is internal, so we use reflection to create and test it
		var assembly = typeof(MongoDbProviderOptions).Assembly;
		var retryPolicyType = assembly.GetType("Excalibur.Data.MongoDB.MongoDbRetryPolicy");
		if (retryPolicyType == null)
		{
			throw new InvalidOperationException("Could not find MongoDbRetryPolicy type");
		}
		...
		if (instance == null)
		{
			throw new InvalidOperationException("Could not create MongoDbRetryPolicy instance");
		}
	...
	{
		// Arrange
		var policy = CreateMongoDbRetryPolicy(3);
	...
	{
		// Arrange
		var policy = CreateMongoDbRetryPolicy(maxRetryAttempts);
	...
	[Fact]
	public void ReturnTrue_ForMongoException()
	{
		// Arrange
		var policy = CreateMongoDbRetryPolicy(3);
	...
	[Fact]
	public void ReturnTrue_ForMongoConnectionException()
	{
		// Arrange
		var policy = CreateMongoDbRetryPolicy(3);
		var exception = new MongoConnectionException(new ConnectionId(new ServerId(new ClusterId(), new DnsEndPoint("localhost", 27017)), 0), "Connection failed");
	...
	[Fact]
	public void ReturnFalse_ForNonMongoException()
	{
		// Arrange
		var policy = CreateMongoDbRetryPolicy(3);
		var exception = new InvalidOperationException("Not a Mongo exception");
	...
	public void RespectMaxRetryAttempts_ForMongoException(int attemptNumber, int maxRetry, bool expectedResult)
	{
		// Arrange
		var policy = CreateMongoDbRetryPolicy(maxRetry);
		var exception = new MongoException("Test");
	...
	{
		// Arrange
		var policy = CreateMongoDbRetryPolicy(10);
	...
	{
		// Arrange & Act
		var policy = CreateMongoDbRetryPolicy(0);
