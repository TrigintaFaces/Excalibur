...
/// <summary>
/// Unit tests for <see cref="MongoDBEventSourcingBuilder"/> — 4 connection overloads,
/// feature methods, last-wins semantics, and fluent chaining.
/// </summary>
[Trait(TraitNames.Category, TestCategories.Unit)]
[Trait(TraitNames.Component, TestComponents.Core)]
[Trait("Database", "MongoDB")]
public sealed class MongoDBEventSourcingBuilderShould : UnitTestBase
{
    private const string TestConnectionString = "mongodb://localhost:27017";

    private static (MongoDBEventSourcingBuilder Builder, MongoDbEventStoreOptions Options) CreateBuilder()
    {
        var options = new MongoDbEventStoreOptions();
        var builder = new MongoDBEventSourcingBuilder(options);
    ...
    public void BindConfiguration_StorePath()
    {
        ...
        builder.BindConfiguration("EventSourcing:MongoDB");

        builder.BindConfigurationPath.ShouldBe("EventSourcing:MongoDB");
    }
    ...
    public void BindConfiguration_ClearOtherOverloads()
    {
        ...
        builder.Client(client);
        builder.BindConfiguration("ES:MongoDB");

        options.ConnectionString.ShouldBe(null!);
        builder.ClientInstance.ShouldBeNull();
        builder.ClientFactoryFunc.ShouldBeNull();
        builder.BindConfigurationPath.ShouldBe("ES:MongoDB");
    }
    ...
    public void CollectionName_SetOnOptions()
    {
        ...
        builder.CollectionName("my_events");

        options.CollectionName.ShouldBe("my_events");
    }

    [Fact]
    public void CounterCollectionName_SetOnOptions()
    {
        var (builder, options) = CreateBuilder();

        builder.CounterCollectionName("my_counters");

        options.CounterCollectionName.ShouldBe("my_counters");
    }
    ...
    {
        var (builder, _) = CreateBuilder();
        var result = builder.BindConfiguration("ES:MongoDB");
        result.ShouldBeSameAs(builder);
    }
    ...
    public void Constructor_ThrowOnNullOptions()
    {
        Should.Throw<ArgumentNullException>(() =>
            new MongoDBEventSourcingBuilder(null!));
    }
    ...
    [InlineData("")]
    [InlineData("   ")]
    public void CounterCollectionName_ThrowOnInvalidValue(string? value)
    {
        var (builder, _) = CreateBuilder();
        Should.Throw<ArgumentException>(() => builder.CounterCollectionName(value!));
    }
}
