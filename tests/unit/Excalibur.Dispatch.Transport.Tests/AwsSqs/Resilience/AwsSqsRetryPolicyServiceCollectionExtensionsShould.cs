// SPDX-FileCopyrightText: Copyright (c) 2026 The Excalibur Project
// SPDX-License-Identifier: LicenseRef-Excalibur-1.0 OR AGPL-3.0-or-later OR SSPL-1.0 OR Apache-2.0

using Excalibur.Dispatch.Transport.AwsSqs;

namespace Excalibur.Dispatch.Transport.Tests.AwsSqs.Resilience;

[Trait("Category", "Unit")]
[Trait("Component", "Transport")]
public sealed class AwsSqsRetryPolicyServiceCollectionExtensionsShould
{
	[Fact]
	public void ThrowWhenServicesIsNull()
	{
		Should.Throw<ArgumentNullException>(() =>
			AwsSqsRetryPolicyServiceCollectionExtensions.AddAwsSqsRetryPolicy(
				null!, _ => { }));
	}

	[Fact]
	public void ThrowWhenConfigureIsNull()
	{
		var services = new ServiceCollection();

		Should.Throw<ArgumentNullException>(() =>
			services.AddAwsSqsRetryPolicy((Action<AwsSqsRetryOptions>)null!));
	}

	[Fact]
	public void RegisterRetryPolicyOptions()
	{
		// Arrange
		var services = new ServiceCollection();

		// Act
		var result = services.AddAwsSqsRetryPolicy(opts =>
		{
			opts.MaxRetryAttempts = 5;
			opts.BaseDelay = TimeSpan.FromMilliseconds(200);
			opts.RetryStrategy = SqsRetryStrategy.Exponential;
		});

		// Assert
		result.ShouldBeSameAs(services);
		services.Count.ShouldBeGreaterThan(0);
	}
}
