...
public sealed class Argon2idPasswordHasherShould
{
	...
	{
		_sut = new Argon2idPasswordHasher(Options.Create(_defaultOptions));
	}
	...
	public async Task VerifyPasswordAsync_WhenParametersChanged_ReturnsSuccessRehashNeeded()
	{
		...
		var oldHasher = new Argon2idPasswordHasher(Options.Create(oldOptions));
		var newHasher = new Argon2idPasswordHasher(Options.Create(newOptions));
	...
	public void Constructor_WithInvalidMemorySize_ThrowsArgumentException(int memorySize)
	{
		...
		_ = Should.Throw<ArgumentException>(
			() => new Argon2idPasswordHasher(Options.Create(options)));
	}
	...
	public async Task HashPasswordAsync_StoresCorrectParameters()
	{
		...
		var hasher = new Argon2idPasswordHasher(Options.Create(customOptions));
		var password = "SecurePassword123!";
