...
/// <summary>
/// Unit tests for <see cref="RedisOutboxBuilder"/> — 4 connection overloads,
/// feature methods, last-wins semantics, and fluent chaining.
/// </summary>
[Trait(TraitNames.Category, TestCategories.Unit)]
[Trait(TraitNames.Component, TestComponents.Core)]
[Trait("Database", "Redis")]
public sealed class RedisOutboxBuilderShould : UnitTestBase
{
    private const string TestConnectionString = "localhost:6379";

    private static (RedisOutboxBuilder Builder, RedisOutboxOptions Options) CreateBuilder()
    {
        var options = new RedisOutboxOptions();
        var builder = new RedisOutboxBuilder(options);
    ...
    public void BindConfiguration_StorePath()
    {
        ...
        builder.BindConfiguration("Outbox:Redis");

        builder.BindConfigurationPath.ShouldBe("Outbox:Redis");
    }
    ...
    public void KeyPrefix_SetOnOptions()
    {
        ...
        builder.KeyPrefix("myoutbox");

        options.KeyPrefix.ShouldBe("myoutbox");
    }
    ...
    {
        var (builder, _) = CreateBuilder();
        var result = builder.BindConfiguration("Outbox:Redis");
        result.ShouldBeSameAs(builder);
    }
