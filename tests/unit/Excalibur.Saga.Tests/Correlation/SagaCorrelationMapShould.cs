...
/// <summary>
/// Unit tests for <see cref="SagaCorrelationMap{TSaga}"/>.
/// </summary>
[Trait("Category", "Unit")]
[Trait("Component", "Saga")]
public sealed class SagaCorrelationMapShould
{
	[Fact]
	public void TryGetCorrelationId_ShouldReturnTrue_WhenMessageTypeIsRegistered()
	{
		...
		var config = new SagaCorrelationMap<TestSaga>(correlators);
	...
	{
		// Arrange
		var config = new SagaCorrelationMap<TestSaga>(new Dictionary<Type, Func<object, string>>());
	...
	{
		Should.Throw<ArgumentNullException>(
			() => new SagaCorrelationMap<TestSaga>(null!));
	}
