...
/// <summary>
/// Unit tests for <see cref="ReadAllGrantsRequest"/>.
/// Covers SQL structure, parameter setup, and command timeout.
/// </summary>
[Trait("Category", "Unit")]
[Trait("Component", "Postgres")]
public sealed class ReadAllGrantsRequestShould
{
	...
	{
		// Act
		var request = new ReadAllGrantsRequest(UserId, CancellationToken.None);

		// Assert
		request.Command.CommandText.ShouldNotBeNullOrWhiteSpace();
		request.ResolveAsync.ShouldNotBeNull();
		request.RequestType.ShouldBe(nameof(ReadAllGrantsRequest));
	}
