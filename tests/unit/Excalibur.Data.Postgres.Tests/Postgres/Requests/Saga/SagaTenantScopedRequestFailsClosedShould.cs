...
// Independent regression lock (author != implementer) for the Postgres saga request tenant-discriminator
...
// (SaveSagaRequest emits the conditional gate only on its version-gated UPDATE branch — an EXISTING saga,
// Version > 0 — so the factory below saves an existing saga; a new-saga INSERT has no per-row predicate.)
...
public sealed class SagaTenantScopedRequestFailsClosedShould
{
	...
	// Each tenant-columned saga request DTO as a factory returning its generated SQL. The scoped-only gate is
	// uniform across Postgres requests: "AND tenant_id = @TenantId".
	private const string ScopedGate = "AND tenant_id = @TenantId";

	private static readonly (string Name, Func<TenantScope, string> CommandTextFor)[] TenantColumnedRequests =
	[
		("LoadSagaRequest",
			s => new LoadSagaRequest<TestSagaState>(Guid.NewGuid(), CreateOptions(), new DispatchJsonSerializer(), s, Ct).Command.CommandText),
		("SaveSagaRequest",
			// Version 3 => the version-gated UPDATE branch, which carries the conditional tenant gate (a
			// Version-0 INSERT has no per-row predicate).
			s => new SaveSagaRequest<TestSagaState>(new TestSagaState { SagaId = Guid.NewGuid(), Version = 3 }, CreateOptions(), new DispatchJsonSerializer(), s, Ct).Command.CommandText),
		("GetSagaSummaryRequest",
			s => new GetSagaSummaryRequest(Guid.NewGuid(), CreateOptions(), s, Ct).Command.CommandText),
	];
	...
	public void FailClosed_ThrowingTenantRequired_WhenScopedWithNoTenant(string? tenantId)
	{
		...
		// context — fails closed identically rather than emitting a predicate-less query.
	...
	public void EmitTheTenantGate_EvenWhenUnscoped_ForEverySagaRequest()
	{
		...
		// happened to carry that id. Omission widened the read, exactly as it did on the audit and data
		// inventory paths.
		//
		// The old arm's stated fear — "an impl that emits the gate unconditionally breaks every non-MT
		// deployment" — is answered by the sentinel rather than by dropping the predicate: an untenanted
		// deployment writes and reads the reserved value, so the gate always matches its own partition and
		// never spans tenants. That is what makes unconditional safe, and it is why this inverts rather
		// than being deleted: the ungated path must stay unreachable.
