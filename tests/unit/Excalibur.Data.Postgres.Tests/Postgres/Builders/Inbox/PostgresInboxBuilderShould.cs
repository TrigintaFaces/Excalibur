...
/// <summary>
/// Unit tests for <see cref="PostgresInboxBuilder"/> — 5 connection overloads,
/// feature methods, last-wins semantics, and fluent chaining.
/// </summary>
[Trait(TraitNames.Category, TestCategories.Unit)]
[Trait(TraitNames.Component, TestComponents.Core)]
[Trait("Database", "Postgres")]
public sealed class PostgresInboxBuilderShould : UnitTestBase
{
    private const string TestConnectionString =
        "Host=localhost;Database=TestDb;Username=test;Password=test";

    private static (PostgresInboxBuilder Builder, PostgresInboxOptions Options) CreateBuilder()
    {
        var options = new PostgresInboxOptions();
        var builder = new PostgresInboxBuilder(options);
    ...
    public void ConnectionStringName_StoreName()
    {
        ...
        builder.ConnectionStringName("InboxStore");

        builder.ConnectionStringNameValue.ShouldBe("InboxStore");
    }
    ...
    public void BindConfiguration_StorePath()
    {
        ...
        builder.BindConfiguration("Inbox:Postgres");

        builder.BindConfigurationPath.ShouldBe("Inbox:Postgres");
    }
    ...
    {
        var (builder, options) = CreateBuilder();
        builder.TableName("custom_inbox");
        options.TableName.ShouldBe("custom_inbox");
    }

    [Fact]
    public void MaxRetryCount_SetValueOnOptions()
    {
        var (builder, options) = CreateBuilder();
        builder.MaxRetryCount(5);
        options.MaxRetryCount.ShouldBe(5);
    }

    [Fact]
    public void MaxRetryCount_AllowZero()
    {
        var (builder, options) = CreateBuilder();
        builder.MaxRetryCount(0);
    ...
    {
        var (builder, _) = CreateBuilder();
        var result = builder.ConnectionStringName("InboxStore");
        result.ShouldBeSameAs(builder);
    }
    ...
    {
        var (builder, _) = CreateBuilder();
        var result = builder.BindConfiguration("Inbox:Postgres");
        result.ShouldBeSameAs(builder);
    }
