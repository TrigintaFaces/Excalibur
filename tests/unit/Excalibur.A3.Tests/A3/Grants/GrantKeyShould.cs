...
public sealed class GrantKeyShould : UnitTestBase
{
	...
	public void Create_FromKeyString_WithTooFewParts_ThrowsArgumentException()
	{
		...
		// names the segment count found and the count expected, which says why the key cannot be decoded
		// rather than only that it is wrong.
		ex.Message.ShouldContain("3 segment(s)");
		ex.Message.ShouldContain("4 were expected");
	}
