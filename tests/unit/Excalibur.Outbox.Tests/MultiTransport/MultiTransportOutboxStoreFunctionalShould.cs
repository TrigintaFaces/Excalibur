...
public sealed class MultiTransportOutboxStoreFunctionalShould
{
	...
	public async Task PublishToTransportAsync_ShouldSetTargetTransportAndDelegate()
	{
		...
		await store.PublishToTransportAsync("kafka", message, CancellationToken.None);
	...
	public async Task PublishToTransportsAsync_ShouldSetMultiTransportAndDelegate()
	{
		...
		await store.PublishToTransportsAsync(transports, message, CancellationToken.None);
	...
	public async Task EnqueueAsync_ShouldDelegateToInnerStore()
	{
		...
		await store.EnqueueAsync(message, context, CancellationToken.None);
	...
	public async Task GetUnsentMessagesAsync_ShouldDelegateToInnerStore()
	{
		...
		var result = await store.GetUnsentMessagesAsync(10, CancellationToken.None);
	...
	public async Task MarkFailedAsync_ShouldDelegateToInnerStore()
	{
		...
		await store.MarkFailedAsync("msg-1", "error", 2, CancellationToken.None);
