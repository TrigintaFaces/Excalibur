...
/// <c>ReadRetryableEntriesAsync</c> re-fetched failed entries via
/// <see cref="IInboxStoreAdmin.GetAllTenantsFailedEntriesAsync(int, System.DateTimeOffset?, int, System.Threading.CancellationToken)"/>
...
/// by invoking the private <c>ReadRetryableEntriesAsync(int, CancellationToken)</c> via reflection —
...
{
	private static readonly MethodInfo ReadRetryableEntriesAsyncMethod =
		typeof(InboxProcessor).GetMethod("ReadRetryableEntriesAsync", BindingFlags.NonPublic | BindingFlags.Instance)
		?? throw new InvalidOperationException("Expected private InboxProcessor.ReadRetryableEntriesAsync(int, CancellationToken).");
	...
	public async Task PassConfiguredMaxAttemptsAsTheReFetchCeiling_NotAHardcodedLiteral()
	{
		...
		// Act
		await InvokeReadRetryableEntriesAsync(processor).ConfigureAwait(false);
	...
	private static async Task InvokeReadRetryableEntriesAsync(InboxProcessor processor)
	{
		// ReadRetryableEntriesAsync(int batchSize, CancellationToken cancellationToken) -> Task<IReadOnlyCollection<InboxEntry>>
		var task = (Task)ReadRetryableEntriesAsyncMethod.Invoke(processor, [50, CancellationToken.None])!;
		await task.ConfigureAwait(false);
	}

	/// <summary>
	/// Captures the <c>maxRetries</c> argument passed to the store's <c>GetAllTenantsFailedEntriesAsync</c>.
	/// </summary>
