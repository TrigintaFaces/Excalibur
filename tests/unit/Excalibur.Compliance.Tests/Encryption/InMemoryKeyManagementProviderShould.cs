...
public sealed class InMemoryKeyManagementProviderShould : IDisposable
{
	...
	// nbck06: this previously asserted the fail-open-to-throw shape -- a purpose-scoped lookup that finds
	// nothing returned null EVEN WITH AutoGenerateDefaultKey on, so a decorator resolving its own
	// EncryptionOptions.DefaultPurpose ("default") against a fresh, zero-config provider threw
	// EncryptionException on the very first encrypt. The bootstrap key is generated with no purpose of its
	// own (nothing has been configured yet when it is created), so it can never satisfy the purpose filter
	// above it; restricting the fallback to purpose is null made it unreachable from every real caller,
	// which always resolves SOME purpose string. Inverted to the contract that replaced it: with
	// auto-generation on and nothing else configured, ANY purpose falls back to the bootstrap key.
	[Fact]
	public async Task Fall_back_to_the_auto_generated_key_when_no_active_key_matches_the_purpose()
	{
		// Act
	...
	// LIVENESS's safety pair: the fallback is conditional on AutoGenerateDefaultKey, not unconditional --
	// with it explicitly off, an unmatched purpose still returns null (the provider makes no convenience
	// promise to keep, and returning a wrong-purpose key when the caller deliberately disabled the
	// bootstrap key would be a silent purpose-separation violation, not a convenience).
	[Fact]
	public async Task Return_null_when_no_active_key_for_purpose_and_auto_generation_is_disabled()
	{
		// Arrange
		using var sut = new InMemoryKeyManagementProvider(
			NullLogger<InMemoryKeyManagementProvider>.Instance,
		...
		// Act
		var key = await sut.GetActiveKeyAsync("nonexistent-purpose", CancellationToken.None).ConfigureAwait(false);
