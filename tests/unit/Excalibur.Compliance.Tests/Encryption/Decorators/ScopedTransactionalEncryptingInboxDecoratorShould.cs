...
/// <see cref="EncryptingInboxStoreDecorator"/> must NOT silently downgrade the exactly-once guarantee.
/// </summary>
/// <remarks>
/// <para>
...
public sealed class ScopedTransactionalEncryptingInboxDecoratorShould
{
	...
	public async Task Be_selected_by_the_middleware_on_the_scoped_transactional_path()
	{
		...
		_ = observedScope.ShouldNotBeNull(
			...
			+ "the encryption-decorated document store was silently downgraded to the weaker at-least-once claim path");
	...
	public async Task Roll_back_both_the_mark_and_the_enlisted_write_when_the_handler_throws()
	{
		...
		var fresh = new ScopedTransactionalInboxStoreDouble(backing);
		(await fresh.IsProcessedAsync(MessageId, HandlerType, CancellationToken.None))
			.ShouldBeFalse("a thrown handler must leave no processed-mark behind the encryption decorator");
	...
	{
		var sut = (IInboxStoreCapabilities)Decorate(A.Fake<IInboxStore>());
	...
	private const string MessageId = "scoped-encrypting-msg";
	private static readonly string HandlerType = typeof(EncryptingScopedProbeMessage).FullName!;
	...
	private static async Task<IMessageResult> InvokeAsync(InboxMiddleware middleware, Func<IMessageContext, bool> handler)
	{
		...
		return await middleware.InvokeAsync(new EncryptingScopedProbeMessage(), context, next, CancellationToken.None);
	}
...
internal sealed class EncryptingScopedProbeMessage : IDispatchMessage;
