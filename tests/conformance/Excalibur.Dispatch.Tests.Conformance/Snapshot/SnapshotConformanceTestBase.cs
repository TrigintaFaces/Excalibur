...
public abstract class SnapshotConformanceTestBase : IAsyncLifetime
{
	...
	public virtual async Task Should_Not_Fault_When_Many_Callers_Use_The_Store_Concurrently()
	{
		...
		await ConcurrentFirstUse.ShouldNotFaultAsync(
			async () => _ = await store.GetLatestSnapshotAsync(absent, "ConcurrentFirstUse", CancellationToken.None).ConfigureAwait(false),
			"the snapshot store").ConfigureAwait(false);
	}
}
