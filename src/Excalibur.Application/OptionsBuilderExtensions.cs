...
public static class OptionsBuilderExtensions
{
	...
	/// <remarks>
	///     This extension integrates FluentValidation into the options validation process. It supports rule sets if the validator
	///     implements <see cref="IDetermineValidationRuleSetByConfig{T}" />.
	/// </remarks>
		...
		where TValidator : IValidator<TOptions>, new()
	{
		...
		bool ValidateOptionsInstance(TOptions optionsInstance)
		{
			...
			var ruleSetDiscriminator = (IDetermineValidationRuleSetByConfig<TOptions>)validator;

			// Perform validation
			var result = validator.Validate(optionsInstance, options =>
			{
				_ = options.IncludeRuleSets(ruleSetDiscriminator?.WhichRuleSets(optionsInstance));
			});
			...
			var message = $"Validation failed for {typeof(TOptions).FullName}: {string.Join(string.Empty, errors)}";
