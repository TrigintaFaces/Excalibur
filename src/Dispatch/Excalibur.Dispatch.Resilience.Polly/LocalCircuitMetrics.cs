...
/// <summary>
/// The failure/success counters and rolling sampling window backing one circuit breaker instance's
/// open decision. Process-local by design: see the remarks.
/// </summary>
/// <remarks>
/// These counters are deliberately NOT shared between instances. Sharing them would require an atomic
/// increment or a compare-and-swap against the coordinating store, and
/// <see cref="Microsoft.Extensions.Caching.Distributed.IDistributedCache"/> exposes neither — only Get,
/// Set and Remove. A shared counter maintained by an unsynchronised read-modify-write loses updates
/// between instances, so the breaker under-counts and opens late or never — the failure a circuit
/// breaker exists to prevent. Counting locally and sharing only the resulting circuit STATE (a
/// single-value write that is correct under last-writer-wins) keeps every count exact.
/// </remarks>
internal sealed class LocalCircuitMetrics
{
	/// <summary>
 ...
 bucket.Failures++;
	...
	/// <summary>
	/// Clears every counter and rolling-window bucket. Called when the circuit closes so a freshly
	/// closed circuit starts a clean window rather than resuming the run that preceded the trip.
	/// </summary>
	public void Reset()
	{
		SuccessCount = 0;
