...
/// tenant IDs, dynamic queue names or aggregate identifiers) that would cause O(n) metric
/// scrape cost or excessive trace attribute cardinality.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// Lives in the abstractions package so that every package emitting metrics guards its tag
/// dimensions the same way, and so that a consumer writing their own transport, store or
/// middleware can apply the same limit to the tags they emit.
/// </para>
/// <para>
/// Thread-safe via <see cref="ConcurrentDictionary{TKey, TValue}"/>. O(1) amortized per call.
/// The dictionary is bounded to the configured maximum cardinality; once full, new values are
/// mapped to the overflow sentinel without growing the dictionary.
/// </para>
/// </remarks>
