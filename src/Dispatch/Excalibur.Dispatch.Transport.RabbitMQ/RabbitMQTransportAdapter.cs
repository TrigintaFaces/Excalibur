...
/// <summary>
/// RabbitMQ transport adapter that wraps the existing RabbitMqMessageBus infrastructure.
/// </summary>
/// <remarks>
/// <para>
/// This adapter provides integration with the unified transport configuration system
/// while delegating actual RabbitMQ operations to the existing <see cref="RabbitMqMessageBus"/>.
/// </para>
/// <para>
/// For publishing messages, the adapter uses RabbitMqMessageBus.
/// For receiving messages, use the existing RabbitMqChannelConsumer infrastructure.
/// </para>
/// </remarks>
internal sealed partial class RabbitMQTransportAdapter : ITransportAdapter, ITransportAdapterLifecycle, ITransportHealthChecker, ITransportHealthMetrics, IAsyncDisposable
{
	/// <summary>
	...
	/// </summary>
	public const string TransportTypeName = "rabbitmq";

	private readonly ILogger<RabbitMQTransportAdapter> _logger;
	private readonly RabbitMqMessageBus _messageBus;
	private readonly RabbitMQTransportAdapterOptions _options;
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="RabbitMQTransportAdapter"/> class.
	/// </summary>
	/// <param name="logger"> The logger instance. </param>
	/// <param name="messageBus"> The RabbitMQ message bus to wrap. </param>
	...
	public RabbitMQTransportAdapter(
		...
		RabbitMQTransportAdapterOptions? options = null)
	{
		_logger = logger ?? throw new ArgumentNullException(nameof(logger));
		_messageBus = messageBus ?? throw new ArgumentNullException(nameof(messageBus));
		_serviceProvider = serviceProvider ?? throw new ArgumentNullException(nameof(serviceProvider));
		_options = options ?? new RabbitMQTransportAdapterOptions();
	}
		...
		CancellationToken cancellationToken)
	{
		...
		if (!IsRunning)
		{
			...
			return MessageResult.Failed(new MessageProblemDetails
			{
				...
				ErrorCode = 10001,
				Detail = "The RabbitMQ transport adapter is not running",
				Instance = $"rabbitmq-adapter-{Guid.NewGuid()}",
			});
		}
		...
		CancellationToken cancellationToken)
	{
		...
		{
			TransportMeter.RecordError(Name, TransportType, "not_running");
			throw new InvalidOperationException("The RabbitMQ transport adapter is not running");
		}
	...
	public Task<TransportHealthCheckResult> CheckQuickHealthAsync(CancellationToken cancellationToken)
	{
		...
		var description = IsRunning
			? "RabbitMQ transport adapter is running"
			: "RabbitMQ transport adapter is not running";
	...
	[LoggerMessage(RabbitMqEventId.TransportAdapterStarting, LogLevel.Information,
		"Starting RabbitMQ transport adapter")]
	private partial void LogStarting();

	[LoggerMessage(RabbitMqEventId.TransportAdapterStopping, LogLevel.Information,
		"Stopping RabbitMQ transport adapter")]
	private partial void LogStopping();

	[LoggerMessage(RabbitMqEventId.ReceivingMessage, LogLevel.Debug,
	...
	[LoggerMessage(RabbitMqEventId.SendingMessage, LogLevel.Debug,
	...
	[LoggerMessage(RabbitMqEventId.MessageProcessingFailed, LogLevel.Error,
		"Failed to process message {MessageId}")]
	private partial void LogMessageProcessingFailed(string messageId, Exception ex);

	[LoggerMessage(RabbitMqEventId.SendFailed, LogLevel.Error,
