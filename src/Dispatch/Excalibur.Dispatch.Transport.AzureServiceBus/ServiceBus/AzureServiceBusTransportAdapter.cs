...
/// <summary>
/// Azure Service Bus transport adapter that wraps the existing AzureServiceBusMessageBus infrastructure.
/// </summary>
/// <remarks>
/// <para>
/// This adapter provides integration with the unified transport configuration system
/// while delegating actual Azure Service Bus operations to the existing <see cref="AzureServiceBusMessageBus"/>.
/// </para>
/// <para>
...
public sealed partial class AzureServiceBusTransportAdapter : ITransportAdapter, ITransportHealthChecker, IAsyncDisposable
{
	...
	/// </summary>
	public const string DefaultName = "AzureServiceBus";

	/// <summary>
	/// The transport type identifier.
	/// </summary>
	public const string TransportTypeName = "azure-servicebus";

	private readonly ILogger<AzureServiceBusTransportAdapter> _logger;
	private readonly AzureServiceBusMessageBus _messageBus;
	private readonly AzureServiceBusTransportAdapterOptions _options;
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="AzureServiceBusTransportAdapter"/> class.
	/// </summary>
	/// <param name="logger"> The logger instance. </param>
	/// <param name="messageBus"> The Azure Service Bus message bus to wrap. </param>
	...
	public AzureServiceBusTransportAdapter(
		...
		AzureServiceBusMessageBus messageBus,
		IServiceProvider serviceProvider,
		AzureServiceBusTransportAdapterOptions? options = null)
	{
		_logger = logger ?? throw new ArgumentNullException(nameof(logger));
		_messageBus = messageBus ?? throw new ArgumentNullException(nameof(messageBus));
		_serviceProvider = serviceProvider ?? throw new ArgumentNullException(nameof(serviceProvider));
		_options = options ?? new AzureServiceBusTransportAdapterOptions();
	}
		...
		CancellationToken cancellationToken)
	{
		...
		if (!IsRunning)
		{
			...
			return MessageResult.Failed(new MessageProblemDetails
			{
				...
				ErrorCode = 10001,
				Detail = "The Azure Service Bus transport adapter is not running",
				Instance = $"azure-servicebus-adapter-{Guid.NewGuid()}",
			});
		}
		...
		CancellationToken cancellationToken)
	{
		...
		{
			TransportMeter.RecordError(Name, TransportType, "not_running");
			throw new InvalidOperationException("The Azure Service Bus transport adapter is not running");
		}
		...
		catch (Exception ex) when (ex is not ArgumentException and not InvalidOperationException)
		{
			...
			TransportMeter.RecordSendDuration(Name, TransportType, stopwatch.Elapsed.TotalMilliseconds);
			throw new InvalidOperationException($"Failed to send message to Azure Service Bus: {ex.Message}", ex);
		}
	}
		...
		CancellationToken cancellationToken)
	{
		...
		if (!IsRunning)
		{
			result = TransportHealthCheckResult.Unhealthy(
				"Azure Service Bus transport adapter is not running",
		...
		else if (failed > 0 && failed > successful / 10)
		{
			result = TransportHealthCheckResult.Degraded(
				$"Azure Service Bus transport has elevated failure rate: {failed}/{total}",
				context.RequestedCategories,
				stopwatch.Elapsed,
				data);
		}
		else
		{
			result = TransportHealthCheckResult.Healthy(
				"Azure Service Bus transport adapter is healthy and running",
				context.RequestedCategories,
				stopwatch.Elapsed,
				data);
		}
	...
	public Task<TransportHealthCheckResult> CheckQuickHealthAsync(CancellationToken cancellationToken)
	{
		...
		var description = IsRunning
			? "Azure Service Bus transport adapter is running"
			: "Azure Service Bus transport adapter is not running";
	...
	[LoggerMessage(AzureServiceBusEventId.TransportAdapterStarting, LogLevel.Information,
		"Starting Azure Service Bus transport adapter")]
	private partial void LogStarting();

	[LoggerMessage(AzureServiceBusEventId.TransportAdapterStopping, LogLevel.Information,
		"Stopping Azure Service Bus transport adapter")]
	private partial void LogStopping();

	[LoggerMessage(AzureServiceBusEventId.TransportReceivingMessage, LogLevel.Debug,
	...
	[LoggerMessage(AzureServiceBusEventId.TransportSendingMessage, LogLevel.Debug,
	...
	[LoggerMessage(AzureServiceBusEventId.TransportMessageProcessingFailed, LogLevel.Error,
		"Failed to process message {MessageId}")]
	private partial void LogMessageProcessingFailed(string messageId, Exception ex);

	[LoggerMessage(AzureServiceBusEventId.TransportSendFailed, LogLevel.Error,
...
public sealed class AzureServiceBusTransportAdapterOptions
{
	...
	/// </summary>
	/// <value> The transport name. Default is "AzureServiceBus". </value>
