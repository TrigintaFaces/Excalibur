...
/// <summary>
/// Azure Event Hubs transport adapter that wraps the existing AzureEventHubMessageBus infrastructure.
/// </summary>
/// <remarks>
/// <para>
/// This adapter provides integration with the unified transport configuration system
/// while delegating actual Azure Event Hubs operations to the existing <see cref="AzureEventHubMessageBus"/>.
/// </para>
/// <para>
/// For publishing messages, the adapter uses AzureEventHubMessageBus.
/// </para>
/// </remarks>
internal sealed partial class AzureEventHubsTransportAdapter : ITransportAdapter, ITransportAdapterLifecycle, ITransportHealthChecker, IAsyncDisposable
{
	/// <summary>
	/// The default transport name for Azure Event Hubs adapters.
	/// </summary>
	public const string DefaultName = "AzureEventHubs";

	/// <summary>
	/// The transport type identifier.
	/// </summary>
	public const string TransportTypeName = "azure-eventhubs";

	private readonly ILogger<AzureEventHubsTransportAdapter> _logger;
	private readonly AzureEventHubMessageBus _messageBus;
	private readonly AzureEventHubsTransportAdapterOptions _options;
	...
	/// <param name="messageBus"> The Azure Event Hubs message bus to wrap. </param>
	/// <param name="serviceProvider"> The service provider for resolving dependencies. </param>
	/// <param name="options"> The adapter options. </param>
	public AzureEventHubsTransportAdapter(
		ILogger<AzureEventHubsTransportAdapter> logger,
		AzureEventHubMessageBus messageBus,
		IServiceProvider serviceProvider,
		AzureEventHubsTransportAdapterOptions? options = null)
	{
		_logger = logger ?? throw new ArgumentNullException(nameof(logger));
		_messageBus = messageBus ?? throw new ArgumentNullException(nameof(messageBus));
		_serviceProvider = serviceProvider ?? throw new ArgumentNullException(nameof(serviceProvider));
		_options = options ?? new AzureEventHubsTransportAdapterOptions();
	}
		...
		CancellationToken cancellationToken)
	{
		...
		if (!IsRunning)
		{
			...
			return MessageResult.Failed(new MessageProblemDetails
			{
				...
				ErrorCode = 10001,
				Detail = "The Azure Event Hubs transport adapter is not running",
				Instance = $"azure-eventhubs-adapter-{Guid.NewGuid()}",
			});
		}
		...
		CancellationToken cancellationToken)
	{
		...
		{
			TransportMeter.RecordError(Name, TransportType, "not_running");
			throw new InvalidOperationException("The Azure Event Hubs transport adapter is not running");
		}
		...
		try
		{
			// Route to appropriate AzureEventHubMessageBus.PublishAsync overload based on message type
			switch (message)
			{
		...
		catch (Exception ex) when (ex is not ArgumentException and not InvalidOperationException)
		{
			...
			TransportMeter.RecordSendDuration(Name, TransportType, stopwatch.Elapsed.TotalMilliseconds);
			throw new InvalidOperationException($"Failed to send message to Azure Event Hubs: {ex.Message}", ex);
		}
	}
		...
		CancellationToken cancellationToken)
	{
		...
		if (!IsRunning)
		{
			result = TransportHealthCheckResult.Unhealthy(
				"Azure Event Hubs transport adapter is not running",
		...
		else if (failed > 0 && failed > successful / 10)
		{
			result = TransportHealthCheckResult.Degraded(
				$"Azure Event Hubs transport has elevated failure rate: {failed}/{total}",
				context.RequestedCategories,
				stopwatch.Elapsed,
				data);
		}
		else
		{
			result = TransportHealthCheckResult.Healthy(
				"Azure Event Hubs transport adapter is healthy and running",
				context.RequestedCategories,
				stopwatch.Elapsed,
				data);
		}
	...
	public Task<TransportHealthCheckResult> CheckQuickHealthAsync(CancellationToken cancellationToken)
	{
		...
		var description = IsRunning
			? "Azure Event Hubs transport adapter is running"
			: "Azure Event Hubs transport adapter is not running";
	...
	[LoggerMessage(AzureServiceBusEventId.EventHubsTransportStopping, LogLevel.Information,
		"Stopping Azure Event Hubs transport adapter")]
	private partial void LogStopping();

	[LoggerMessage(AzureServiceBusEventId.EventHubsTransportReceivingMessage, LogLevel.Debug,
	...
	[LoggerMessage(AzureServiceBusEventId.EventHubsTransportSendingMessage, LogLevel.Debug,
	...
	[LoggerMessage(AzureServiceBusEventId.EventHubsTransportMessageProcessingFailed, LogLevel.Error,
		"Failed to process message {MessageId}")]
	private partial void LogMessageProcessingFailed(string messageId, Exception ex);

	[LoggerMessage(AzureServiceBusEventId.EventHubsTransportSendFailed, LogLevel.Error,
...
internal sealed class AzureEventHubsTransportAdapterOptions
{
	...
	/// </summary>
	/// <value> The transport name. Default is "AzureEventHubs". </value>
