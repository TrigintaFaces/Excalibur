...
public sealed partial class AwsS3ClaimCheckStore : IClaimCheckProvider, IDisposable
{
		...
		CancellationToken cancellationToken)
	{
		...
		// form honoured reference.BlobName verbatim, which let whoever could publish to the consumed queue
		// name ANY object in this bucket and have it fetched with our credentials. Falling back to
		...
		//
		// The midnight problem the BlobName branch existed to solve is solved differently now -- the
		// identifier carries its own date partition, so the key no longer depends on the clock at lookup.
		var key = RequireStorageKey(reference.Id);
		...
		CancellationToken cancellationToken)
	{
		...
		if (!ClaimCheckId.TryGetStorageKey(_claimCheckOptions.IdPrefix, _options.Prefix, reference.Id, out var key))
		{
			return false;
		}
