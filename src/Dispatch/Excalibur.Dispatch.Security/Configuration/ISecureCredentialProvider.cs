// SPDX-FileCopyrightText: Copyright (c) 2026 The Excalibur Project
// SPDX-License-Identifier: LicenseRef-Excalibur-1.0 OR AGPL-3.0-or-later OR SSPL-1.0 OR Apache-2.0


using System.Security;

namespace Excalibur.Dispatch.Security;

/// <summary>
/// Defines the contract for secure credential management.
/// </summary>
public interface ISecureCredentialProvider
{
	/// <summary>
	/// Retrieves a credential by key from configured secure stores.
	/// </summary>
	/// <param name="key">The credential key to retrieve.</param>
	/// <param name="cancellationToken">A token that is observed for cancellation.</param>
	/// <returns>A task that completes with the credential, if available.</returns>
	Task<SecureString?> GetCredentialAsync(string key, CancellationToken cancellationToken);

	/// <summary>
	/// Validates that a credential exists and meets security requirements.
	/// </summary>
	/// <param name="key">The credential key to validate.</param>
	/// <param name="requirements">The requirements the credential must satisfy.</param>
	/// <param name="cancellationToken">A token that is observed for cancellation.</param>
	/// <returns>A task that completes with the validation result.</returns>
	Task<CredentialValidationResult> ValidateCredentialAsync(
		string key,
		CredentialRequirements requirements,
		CancellationToken cancellationToken);

	/// <summary>
	/// Rotates a credential by generating a new value and updating all stores.
	/// </summary>
	/// <param name="key">The credential key to rotate.</param>
	/// <param name="cancellationToken">A token that is observed for cancellation.</param>
	/// <returns>A task that completes when rotation finishes.</returns>
	Task RotateCredentialAsync(string key, CancellationToken cancellationToken);
}
