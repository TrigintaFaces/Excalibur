	...
	IConfiguration configuration) : IWritableCredentialStore, IDisposable
{
	...
	[LoggerMessage(SecurityEventId.AwsSecretsManagerRetrieving, LogLevel.Debug, "Retrieving credential {Key} from AWS Secrets Manager")]
	private partial void LogRetrievingCredential(string key);

	[LoggerMessage(SecurityEventId.AwsSecretsManagerSecretNotFound, LogLevel.Warning, "Secret {Key} not found in AWS Secrets Manager")]
	private partial void LogSecretNotFound(string key);

	[LoggerMessage(SecurityEventId.AwsSecretsManagerRetrieved, LogLevel.Information,
		"Successfully retrieved credential {Key} from AWS Secrets Manager")]
	private partial void LogCredentialRetrieved(string key);

	[LoggerMessage(SecurityEventId.AwsSecretsManagerRetrieveFailed, LogLevel.Error, "Failed to retrieve credential {Key} from AWS Secrets Manager")]
	private partial void LogRetrievalFailed(Exception ex, string key);

	[LoggerMessage(SecurityEventId.AwsSecretsManagerStoring, LogLevel.Debug, "Storing credential {Key} in AWS Secrets Manager")]
	private partial void LogStoringCredential(string key);

	[LoggerMessage(SecurityEventId.AwsSecretsManagerStored, LogLevel.Information, "Successfully stored credential {Key} in AWS Secrets Manager")]
	private partial void LogCredentialStored(string key);

	[LoggerMessage(SecurityEventId.AwsSecretsManagerRequestFailed, LogLevel.Error, "Failed to store credential {Key} in AWS Secrets Manager")]
