...
public sealed partial class TransactionMiddleware : IDispatchMiddleware
{
	...
	private static readonly Func<ILogger, string, bool, string, double, IDisposable?> TransactionLogScope =
		LoggerMessage.DefineScope<string, bool, string, double>(
			"MessageType:{MessageType} TransactionEnabled:{TransactionEnabled} IsolationLevel:{IsolationLevel} TransactionTimeout:{TransactionTimeout}");
	...
	private bool RequiresTransaction(IDispatchMessage message)
	{
		...
		var metadata = GetTransactionTypeMetadata(messageType);

		// Check for explicit bypass attributes
		if (metadata.HasNoTransactionAttribute)
		{
			return false;
		}
		...
		if (metadata.HasRequireTransactionAttribute)
		{
			return true;
		}
		...
		IMessageContext context)
	{
		...
		var metadata = GetTransactionTypeMetadata(message.GetType());
		var transactionAttr = metadata.TransactionAttribute;
	...
	private IDisposable? CreateTransactionLoggingScope(
		...
		TransactionConfiguration config)
	{
		return TransactionLogScope(
			_logger,
			message.GetType().Name,
			true,
			config.IsolationLevel.ToString(),
			config.Timeout.TotalSeconds);
	}
	...
	private readonly record struct TransactionTypeMetadata(
		bool HasNoTransactionAttribute,
		bool HasRequireTransactionAttribute,
