...
internal sealed class BatchMessageContext : IMessageContext
{
	...
	public BatchMessageContext(IList<IMessageContext> contexts)
	{
		...
		{
			// Initialize default values when no primary context
			MessageType = "BatchMessage";
			VersionMetadata = new MessageVersionMetadata();
			ValidationResult = SerializableValidationResult.Success();
			AuthorizationResult = Abstractions.AuthorizationResult.Success();
