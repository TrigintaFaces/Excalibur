...
{
	private readonly ConcurrentDictionary<Type, PropertyInfo[]> _sanitizablePropertyCache = new();
	private readonly ConcurrentDictionary<PropertyInfo, bool> _noSanitizeAttributeCache = new();
	private readonly ConcurrentDictionary<Type, bool> _bypassAttributeCache = new();
	private readonly HashSet<string>? _excludedProperties;
	private readonly HashSet<string>? _bypassTypeNames;
		...
		ILogger<InputSanitizationMiddleware> logger)
	{
		...
		_logger = logger;
		_excludedProperties = _options.ExcludeProperties is { Length: > 0 }
			? new HashSet<string>(_options.ExcludeProperties, StringComparer.Ordinal)
			: null;
		_bypassTypeNames = _options.BypassSanitizationForTypes is { Length: > 0 }
			? new HashSet<string>(_options.BypassSanitizationForTypes, StringComparer.Ordinal)
			: null;
	}
	...
	private bool BypassesSanitization(IDispatchMessage message)
	{
		...
		return _bypassTypeNames?.Contains(messageType.Name) == true;
	}
	...
	private PropertyInfo[] GetSanitizableProperties(Type type)
	{
		...
		{
			var allProperties = key.GetProperties(BindingFlags.Public | BindingFlags.Instance);
			...
			{
				if (allProperties[i] is { CanRead: true, CanWrite: true })
				{
					count++;
				}
			}
