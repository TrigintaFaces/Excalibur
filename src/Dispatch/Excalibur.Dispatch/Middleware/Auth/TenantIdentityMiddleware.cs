...
public sealed partial class TenantIdentityMiddleware : IDispatchMiddleware, IEstablishesTenantContext
{
	...
	private static readonly Func<ILogger, string, string, string, IDisposable?> TenantLogScope =
		LoggerMessage.DefineScope<string, string, string>(
			"TenantId:{TenantId} TenantName:{TenantName} TenantRegion:{TenantRegion}");

	private readonly TenantIdentityOptions _options;
	private readonly Lazy<Regex>? _tenantIdPatternRegex;
		...
		ILogger<TenantIdentityMiddleware> logger)
	{
		...
		_tenantIdPatternRegex = ResolveTenantIdPatternRegex(_options.TenantIdPattern);
	...
	private bool IsValidTenantId(string tenantId)
	{
		...
		if (_tenantIdPatternRegex is not null &&
			!_tenantIdPatternRegex.Value.IsMatch(tenantId))
		{
			return false;
		}
	...
	private static Lazy<Regex>? ResolveTenantIdPatternRegex(string? pattern)
	{
	...
	{
		return TenantLogScope(
			_logger,
			tenantContext.TenantId,
			tenantContext.TenantName ?? string.Empty,
			tenantContext.TenantRegion ?? string.Empty);
	}
