...
public sealed partial class ContractVersionCheckMiddleware : IDispatchMiddleware
{
	...
	private static readonly Func<ILogger, string, string, string, string, IDisposable?> VersionLogScope =
		LoggerMessage.DefineScope<string, string, string, string>(
			"MessageType:{MessageType} MessageVersion:{MessageVersion} SchemaId:{SchemaId} ProducerService:{ProducerService}");
		...
		IMessageContext context)
	{
		...
		var schemaId = GetPropertyValue(context, _options.Headers.SchemaIdHeaderName) ??
					   contractTypeMetadata.AttributeSchemaId ??
	...
	private IDisposable CreateVersionLoggingScope(
		...
		ContractVersionContext versionContext)
	{
		return VersionLogScope(
				_logger,
				versionContext.MessageType,
				versionContext.MessageVersion ?? "unspecified",
				versionContext.SchemaId ?? "unknown",
				versionContext.ProducerService ?? string.Empty)
			?? throw new InvalidOperationException(Resources.ContractVersionCheckMiddleware_LoggerNotInitialized);
	}
	...
	private readonly record struct ContractTypeMetadata(
