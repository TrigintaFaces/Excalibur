...
/// reflection path mis-reads. The string-enum half is not surfaced on the options object at all, so it is
/// checked behaviourally instead: the first enum reachable from a registered event type is serialized
/// through the context and a numeric result is rejected on the same terms.
/// </para>
/// <para>
...
public sealed class AotJsonEventSerializer : IEventSerializer
{
	...
	internal AotJsonEventSerializer(IEventTypeRegistry typeRegistry, JsonSerializerContext jsonContext)
	{
		...
		VerifyWireShapeParity(jsonContext, typeRegistry as EventTypeRegistry);
	...
	private static void VerifyWireShapeParity(JsonSerializerContext jsonContext, EventTypeRegistry? registry)
	{
		const string Remedy =
		...
		VerifyEnumsAreWrittenAsStrings(jsonContext, registry, Remedy);
	}

	// UseStringEnumConverter is the third half of the contract and, unlike the other two, it is not readable
	// off JsonSerializerOptions: measured against a source-generated context declared with and without it,
	// Options.Converters is empty either way and both GetConverter(enumType) and
	// GetTypeInfo(enumType).Converter report the same EnumConverter`1. The setting is only observable in what
	// the context actually writes, so that is what is checked: serialize a DEFINED member of the first enum
	// reachable from a registered event type and require a quoted token. A value that is not a declared
	// member is written as a number in BOTH modes, so Enum.ToObject(type, 0) would report a false divergence
	// on an enum with no zero member -- the member name is read first for that reason.
	private static void VerifyEnumsAreWrittenAsStrings(
		JsonSerializerContext jsonContext,
		EventTypeRegistry? registry,
		string remedy)
	{
		// The check is only as wide as the registered event types. A consumer who registers no event types
		// has nothing whose enums could be mis-read; a custom IEventTypeRegistry cannot enumerate its types
		// through the interface, so it is skipped rather than guessed at.
		if (registry is null)
		{
			return;
		}

		foreach (var eventType in registry.RegisteredTypes)
		{
			if (jsonContext.GetTypeInfo(eventType) is not { } eventTypeInfo)
			{
				continue;
			}

			foreach (var property in eventTypeInfo.Properties)
			{
				var propertyType = Nullable.GetUnderlyingType(property.PropertyType) ?? property.PropertyType;
				if (!propertyType.IsEnum || jsonContext.GetTypeInfo(propertyType) is not { } enumTypeInfo)
				{
					continue;
				}

				var memberNames = Enum.GetNames(propertyType);
				if (memberNames.Length == 0)
				{
					continue;
				}

				var written = JsonSerializer.SerializeToUtf8Bytes(
					Enum.Parse(propertyType, memberNames[0]),
					enumTypeInfo);

				if (written.Length > 0 && written[0] == (byte)'"')
				{
					// One enum settles the context-wide setting; the rest would be the same answer.
					return;
				}

				throw new ArgumentException(
					$"The JsonSerializerContext '{jsonContext.GetType().FullName}' writes the enum " +
					$"'{propertyType.FullName}' as a number, so event payloads carrying it cannot be read " +
					"back by the reflection-based event serializer, which writes enums as strings. " + remedy,
					nameof(jsonContext));
			}
		}
	}
