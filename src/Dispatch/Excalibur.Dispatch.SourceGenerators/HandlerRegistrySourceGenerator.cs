...
public sealed class HandlerRegistrySourceGenerator : IIncrementalGenerator
{
	...
	private void GenerateHandlerMetadata(SourceProductionContext context, List<HandlerInfo> handlers)
	{
		...
		_ = sb.AppendLine("// <auto-generated/>");
		_ = sb.AppendLine("#nullable enable");
		_ = sb.AppendLine("// Possible null reference assignment");
#pragma warning disable CS8601
	...
	private void GenerateDirectActionDispatch(SourceProductionContext context, List<HandlerInfo> handlers)
	{
		...
		_ = sb.AppendLine();
		_ = sb.AppendLine(" /// <summary>");
		_ = sb.AppendLine(" /// Self-registers this dispatch table into the framework's PrecompiledDirectDispatchRegistry");
		_ = sb.AppendLine(" /// at module load, so LocalMessageBus can discover it without reflecting over loaded assemblies.");
		_ = sb.AppendLine(" /// </summary>");
		_ = sb.AppendLine(" [ModuleInitializer]");
		_ = sb.AppendLine(" [SuppressMessage(\"Usage\", \"CA2255:The 'ModuleInitializer' attribute should not be used in libraries\",");
		_ = sb.AppendLine("     Justification = \"Required for AOT-safe precompiled direct-action dispatch discovery\")]");
		_ = sb.AppendLine(" public static void Initialize()");
		_ = sb.AppendLine(" {");
		_ = sb.AppendLine(" PrecompiledDirectDispatchRegistry.Register(CanHandle, TryGetMetadata, InvokeAsync);");
		_ = sb.AppendLine(" }");
		_ = sb.AppendLine("}");
	...
	private void GenerateDiscoveredHandlersExtension(SourceProductionContext context, List<HandlerInfo> handlers)
	{
		...
		_ = sb.AppendLine("    /// <param name=\"lifetime\">Service lifetime for registered handlers. Default is <see cref=\"ServiceLifetime.Transient\"/>.</param>");
		...
		_ = sb.AppendLine("        ServiceLifetime lifetime = ServiceLifetime.Transient)");
		_ = sb.AppendLine("    {");
