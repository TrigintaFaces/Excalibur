...
public sealed class MessageResultExtractorGenerator : IIncrementalGenerator
{
	...
	private static string GenerateResultFactoryRegistry(List<ResultTypeInfo> resultTypes)
	{
		...
		// Generate a module initializer that registers factories into the framework's
		// ResultFactoryRegistry via its public RegisterFactory<T>() API.
		_ = sb.AppendLine("/// <summary>");
		_ = sb.AppendLine("/// Generated module initializer that registers AOT-safe result factories");
		_ = sb.AppendLine("/// into the framework's <see cref=\"ResultFactoryRegistry\"/>.");
		_ = sb.AppendLine("/// </summary>");
		_ = sb.AppendLine("[UnconditionalSuppressMessage(\"AOT\", \"IL2026:RequiresUnreferencedCode\",");
		_ = sb.AppendLine("    Justification = \"Generated code registers typed factories, no reflection.\")]");
		_ = sb.AppendLine("[UnconditionalSuppressMessage(\"AOT\", \"IL3050:RequiresDynamicCode\",");
		_ = sb.AppendLine("    Justification = \"Generated code registers typed factories, no runtime code generation.\")]");
		_ = sb.AppendLine("internal static class GeneratedResultFactoryRegistrations");
		_ = sb.AppendLine("{");
		...
		_ = sb.AppendLine("        Justification = \"Required for AOT result factory registration\")]");
		_ = sb.AppendLine("    public static void Initialize()");
		_ = sb.AppendLine("    {");

		foreach (var type in resultTypes)
		{
			_ = sb.AppendLine($"        ResultFactoryRegistry.RegisterFactory<{type.FullTypeName}>();");
		}
