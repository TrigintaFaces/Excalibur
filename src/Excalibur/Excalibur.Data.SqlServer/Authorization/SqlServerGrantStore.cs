...
/// <summary>
/// SQL Server implementation of <see cref="IGrantStore"/> that wraps existing
/// <see cref="SqlServerGrantRequestProvider"/> request classes.
/// </summary>
/// <remarks>
...
public sealed class SqlServerGrantStore : IGrantStore, IGrantQueryStore, IActivityGroupGrantStore
{
	...
	{
		var request = SqlServerGrantRequestProvider.GetGrant(userId, tenantId, grantType, qualifier, cancellationToken);
	...
	{
		var request = SqlServerGrantRequestProvider.GetAllGrants(userId, cancellationToken);
	...
	{
		var request = SqlServerGrantRequestProvider.SaveGrant(grant, cancellationToken);
		return await request.ResolveAsync(_connection).ConfigureAwait(false);
	}
	...
	{
		var request = SqlServerGrantRequestProvider.DeleteGrant(userId, tenantId, grantType, qualifier,
	...
	public async Task<bool> GrantExistsAsync(string userId, string tenantId, string grantType,
		string qualifier, CancellationToken cancellationToken)
	{
		var request = SqlServerGrantRequestProvider.GrantExists(userId, tenantId, grantType, qualifier, cancellationToken);
		return await request.ResolveAsync(_connection).ConfigureAwait(false);
	}
	...
	{
		var request = SqlServerGrantRequestProvider.GetMatchingGrants(userId, tenantId, grantType, qualifier, cancellationToken);
		var result = await request.ResolveAsync(_connection).ConfigureAwait(false);
		return result.ToList().AsReadOnly();
	}
	...
	public async Task<IReadOnlyDictionary<string, object>> FindUserGrantsAsync(string userId,
		CancellationToken cancellationToken)
	{
		var request = SqlServerGrantRequestProvider.FindUserGrants(userId, cancellationToken);
		return await request.ResolveAsync(_connection).ConfigureAwait(false);
	}
	...
	public async Task<int> DeleteActivityGroupGrantsByUserIdAsync(string userId, string grantType,
		CancellationToken cancellationToken)
	{
		var request = SqlServerGrantRequestProvider.DeleteActivityGroupGrantsByUserId(userId, grantType, cancellationToken);
		return await request.ResolveAsync(_connection).ConfigureAwait(false);
	}
	...
	public async Task<int> DeleteAllActivityGroupGrantsAsync(string grantType,
		CancellationToken cancellationToken)
	{
		var request = SqlServerGrantRequestProvider.DeleteAllActivityGroupGrants(grantType, cancellationToken);
		return await request.ResolveAsync(_connection).ConfigureAwait(false);
	}
	...
	{
		var request = SqlServerGrantRequestProvider.InsertActivityGroupGrant(userId, fullName, tenantId,
	...
	{
		var request = SqlServerGrantRequestProvider.GetDistinctActivityGroupGrantUserIds(grantType, cancellationToken);
		var result = await request.ResolveAsync(_connection).ConfigureAwait(false);
		return result.ToList().AsReadOnly();
	}
}
