...
/// <summary>
/// Extension methods for retrieving typed values from data changes in CDC (Change Data Capture) events.
/// </summary>
public static class DataChangeExtensions
{
	...
	/// <summary>
	/// Retrieves the old (previous) value of a specified column from a collection of data changes and converts it to the specified type.
	/// </summary>
	/// <typeparam name="T"> The expected type of the column's old value. </typeparam>
	/// <param name="changes"> The collection of <see cref="DataChange" /> objects representing changes in data. </param>
	/// <param name="columnName"> The name of the column to retrieve the value from. </param>
	/// <param name="defaultValue"> The default value to return if the column is not found or its value is null. </param>
	/// <returns>
	/// The old value of the column converted to type <typeparamref name="T" />, or the <paramref name="defaultValue" /> if the column
	/// is not found or its value is null.
	/// </returns>
	/// <exception cref="InvalidOperationException">
	/// Thrown if the column's value is null and the specified type <typeparamref name="T" /> is not nullable, or if the value cannot be
	/// converted to the specified type.
	/// </exception>
	public static T GetOldValue<T>(this IEnumerable<DataChange> changes, string columnName, T defaultValue = default!)
	{
		var targetType = typeof(T);
		var underlyingType = Nullable.GetUnderlyingType(targetType) ?? targetType;
		var isNullable = Nullable.GetUnderlyingType(targetType) != null || !targetType.IsValueType;
		...
		if (change?.OldValue == null)
		{
			...
			{
				throw new InvalidOperationException(
					$"Column '{columnName}' was not found or its old value is null, and type '{typeof(T)}' is not nullable.");
			}
		...
		{
			throw new InvalidOperationException($"Failed to convert the old value of column '{columnName}' to type '{typeof(T)}'.");
		}
		catch (FormatException)
		{
			throw new InvalidOperationException(
				$"Invalid format for the old value of column '{columnName}' when converting to type '{typeof(T)}'.");
		}
	}

	/// <summary>
	/// Retrieves the new value of a specified column from a data change event and converts it to the specified type.
	/// </summary>
	/// <typeparam name="T"> The expected type of the column's new value. </typeparam>
	/// <param name="changeEvent"> The <see cref="DataChangeEvent" /> containing the changes. </param>
	/// <param name="columnName"> The name of the column to retrieve the value from. </param>
	/// <param name="defaultValue"> The default value to return if the column is not found or its value is null. </param>
	/// <returns>
	/// The new value of the column converted to type <typeparamref name="T" />, or the <paramref name="defaultValue" /> if the column
	/// is not found or its value is null.
	/// </returns>
	public static T GetNewValue<T>(this DataChangeEvent changeEvent, string columnName, T defaultValue = default!)
		=> changeEvent.Changes.GetNewValue(columnName, defaultValue);

	/// <summary>
	/// Retrieves the old (previous) value of a specified column from a data change event and converts it to the specified type.
	/// </summary>
	/// <typeparam name="T"> The expected type of the column's old value. </typeparam>
	/// <param name="changeEvent"> The <see cref="DataChangeEvent" /> containing the changes. </param>
	/// <param name="columnName"> The name of the column to retrieve the value from. </param>
	...
	/// is not found or its value is null.
	/// </returns>
	public static T GetOldValue<T>(this DataChangeEvent changeEvent, string columnName, T defaultValue = default!)
		=> changeEvent.Changes.GetOldValue(columnName, defaultValue);
}
