...
/// <remarks>
/// <para>
/// These options control how the Postgres CDC processor handles scenarios where the saved
/// WAL position is no longer available for consumption. This can occur due to WAL segment removal,
/// replication slot invalidation, or publication changes.
/// </para>
/// <para>
...
{
	/// <summary>
	/// The default maximum number of recovery attempts.
	/// </summary>
	public const int DefaultMaxRecoveryAttempts = 3;

	/// <summary>
	/// The default delay between recovery attempts in seconds.
	/// </summary>
	...
	/// <see cref="PostgresStalePositionException"/> regardless of the configured strategy.
	/// </remarks>
	...
	/// </summary>
	/// <value>
	/// <see langword="true"/> to recreate the slot on invalidation; otherwise, <see langword="false"/>.
	/// Default is <see langword="true"/>.
	/// </value>
	/// <remarks>
	/// When enabled, if the replication slot is detected as invalid (SQLSTATE 55000),
	/// the processor will attempt to drop and recreate the slot before recovering.
	/// </remarks>
