...
public sealed class InMemoryPostgresCdcStateStore : IPostgresCdcStateStore
{
	...
	/// <inheritdoc/>
	public Task<PostgresCdcPosition> GetLastPositionAsync(string processorId, string slotName, CancellationToken cancellationToken)
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(processorId);
		ArgumentException.ThrowIfNullOrWhiteSpace(slotName);

		var key = GetKey(processorId, slotName, null);
		if (_states.TryGetValue(key, out var entry) && PostgresCdcPosition.TryParse(entry.Position, out var position))
		{
	...
	/// <inheritdoc/>
	public Task SavePositionAsync(string processorId, string slotName, PostgresCdcPosition position, CancellationToken cancellationToken)
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(processorId);
		ArgumentException.ThrowIfNullOrWhiteSpace(slotName);
	...
	/// <inheritdoc/>
	public Task<IReadOnlyList<PostgresCdcStateEntry>> GetAllStatesAsync(string processorId, CancellationToken cancellationToken)
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(processorId);
		var entries = _states.Values.Where(e => e.ProcessorId == processorId).OrderBy(e => e.TableName).ToList();
		return Task.FromResult<IReadOnlyList<PostgresCdcStateEntry>>(entries);
	}

	/// <inheritdoc/>
	public Task SaveStateAsync(PostgresCdcStateEntry entry, CancellationToken cancellationToken)
	{
	...
	/// <inheritdoc/>
	public Task ClearStateAsync(string processorId, CancellationToken cancellationToken)
	{
		ArgumentException.ThrowIfNullOrWhiteSpace(processorId);
