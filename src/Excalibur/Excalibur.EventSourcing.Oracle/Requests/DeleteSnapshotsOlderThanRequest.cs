...
public sealed class DeleteSnapshotsOlderThanRequest : DataRequestBase<IDbConnection, int>
{
		...
		string table = "EVENTSTORESNAPSHOTS")
	{
		...
		// sentinel, so both scopes emit the same equality term and the branch is gone. On an age-predicated
		// retention sweep that matters most: this is the statement with the widest reach, and a
		// scope-conditional tenant term is the branch that would let it cross tenants.
