...
/// static factory methods for repository integration with event-sourced repositories.
/// </para>
/// </remarks>
public sealed class Grant : AggregateRoot, IAggregateRoot<Grant, string>
{
	...
	private void Apply(IGrantAdded e)
	{
		UserId = e.UserId;
		FullName = e.FullName;
		Scope = new GrantScope(e.TenantId, e.GrantType, e.Qualifier);
		ExpiresOn = e.ExpiresOn;
		GrantedBy = e.GrantedBy;
		GrantedOn = e.GrantedOn;
		// Set the composite Id from event data
		Id = $"{e.UserId}:{Scope}";
	}

	private void Apply(IGrantRevoked e)
	{
