...
public sealed partial class InMemoryKeyManagementProvider : IKeyManagementProvider, IKeyManagementAdmin, IKeyMaterialProvider, IDisposable
{
		...
		InMemoryKeyManagementOptions? options = null)
	{
		...
		// Deferred key generation using lazy initialization
	...
	public Task<KeyMetadata?> GetActiveKeyAsync(string? purpose, CancellationToken cancellationToken)
	{
		...
		// If no active key matched -- for ANY requested purpose, not only an unspecified one -- and
		// auto-generation is enabled, fall back to the lazily-created bootstrap key. The bootstrap key is
		// generated with no purpose of its own (it exists before the caller has configured any), so a
		// purpose-scoped lookup can never match it by the filter above; restricting this fallback to
		// purpose is null would make it unreachable from every real caller, which always resolves SOME
		// purpose string before asking (EncryptionOptions.DefaultPurpose, "default", when none is set
		// explicitly) -- exactly the mismatch that left AutoGenerateDefaultKey's "just works out of the
		// box" promise unkept on the very first encrypt.
		if (_options.AutoGenerateDefaultKey)
		{
