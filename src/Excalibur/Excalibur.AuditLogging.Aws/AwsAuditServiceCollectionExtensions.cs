...
public static class AwsAuditServiceCollectionExtensions
{
	...
	private static void RegisterAwsAuditExporterCore(IServiceCollection services)
	{
		...
		var httpClientBuilder = services.AddHttpClient<AwsCloudWatchAuditExporter>(static (_, client) =>
		{
		...
		_ = services.AddOptions<HttpStandardResilienceOptions>(httpClientBuilder.Name)
			.Configure<IOptions<AwsAuditOptions>>(static (resilience, exporterOptions) =>
			{
		...
		_ = services.AddTransient<IAuditLogExporter>(static sp => sp.GetRequiredService<AwsCloudWatchAuditExporter>());
	}
}
