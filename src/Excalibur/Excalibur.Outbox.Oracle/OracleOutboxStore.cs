...
using Excalibur.Outbox.Oracle.Diagnostics;
...
public sealed partial class OracleOutboxStore : IOutboxStore, IFencedOutboxStore, IOutboxStoreCapabilities, IOutboxStoreAdmin, IDeadLetterableOutboxStore, IBackoffSchedulableOutboxStore, ITransactionalOutboxWriter, IDisposable, ITenantPartitionedStore
{
	...
	/// <inheritdoc />
	/// <remarks>
	/// The Oracle outbox deletes a message the instant it is marked sent (<see cref="MarkSentAsync(string, CancellationToken)"/>
	...
	public async Task<int> SaveMessagesAsync(ICollection<IOutboxMessage> outboxMessages, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			foreach (var message in outboxMessages)
			{
				...
				// OperationFailedException, so the underlying ORA-00001 unique violation arrives wrapped rather
				// than as a bare OracleException. Unwrap it so a duplicate MessageId surfaces as the uniform
				// InvalidOperationException the IOutboxStore contract specifies (parity with SqlServer/Postgres).
				catch (OperationFailedException ex)
					when (ex.InnerException is OracleException { Number: 1 } inner)
				{
					throw new InvalidOperationException(
						$"Outbox message '{message.MessageId}' already exists.",
						inner);
		...
		CancellationToken cancellationToken)
	{
		...
		if (transaction is not OracleTransaction)
		{
			throw new ArgumentException(
				$"Expected OracleTransaction but received {transaction.GetType().Name}. " +
				"The Oracle outbox store requires an OracleTransaction for transactional staging.",
				nameof(transaction));
		}
	...
	public async ValueTask EnqueueAsync(IDispatchMessage message, IMessageContext context, CancellationToken cancellationToken)
	{
		...
		// Reject an empty/whitespace MessageId on the stage path: Oracle folds an empty string to NULL, which
		// defeats the message_id unique key and leaves the row unmarkable (the empty-string=NULL class the
		// Inbox/Saga stores guard against). The message id is load-bearing identity, so fail fast here.
		ArgumentException.ThrowIfNullOrWhiteSpace(context.MessageId);
	...
	/// rehydrating the metadata header blob and the concrete routing fields. A row that fails conversion is
	/// logged and skipped.
	/// </summary>
	[RequiresUnreferencedCode(
	...
	private List<OutboundMessage> ConvertReservedToOutbound(IEnumerable<IOutboxMessage> reservedMessages)
	{
		...
		foreach (var msg in reservedMessages)
		{
			try
			{
				...
				// CorrelationId/CausationId plus the routing fields (PartitionKey/GroupKey/TargetTransports/
				// IsMultiTransport) are read from the concrete OutboxMessage (populated by the reserve SELECT's
				// dedicated columns); the IOutboxMessage contract does not expose them. Headers round-trip via
				// the metadata blob (deserialized above).
	...
	public async ValueTask MarkSentAsync(string messageId, CancellationToken cancellationToken)
	{
		...
		// For Oracle implementation, we delete the message when it's sent
	...
	/// <inheritdoc />
	/// <remarks>
	/// The delete-on-sent Oracle outbox deletes the winning rows on mark-sent, so a live-row
	/// <c>MAX(fencing_token)</c> cannot survive the drain. The fencing high-water is therefore kept in a
	/// dedicated single-row-per-scope control table (<see cref="OracleOutboxStoreOptions.FenceTableName"/>)
	/// and enforced by an atomic <c>MERGE</c> compare-and-swap before the claim runs. A stale token yields
	/// zero claimable rows — a set-based, non-throwing signal — while the high-water advances monotonically
	/// to the maximum of its current value and the presented token.
	/// </remarks>
	[RequiresUnreferencedCode(
		"JSON deserialization of message payload and metadata may reference types not preserved during trimming. Ensure all deserialized types are annotated with DynamicallyAccessedMembers.")]
	[RequiresDynamicCode(
		...
		CancellationToken cancellationToken)
	{
		...
		// The fence-CAS and the claim run in ONE server-side PL/SQL block that advances the fence under its
		// scope-row lock and stamps the reservation ONLY when the presented token is the high-water — so no
	...
	/// <inheritdoc />
	/// <remarks>
	/// Enforces the fencing high-water and deletes the row in a SINGLE PL/SQL block (fence compare-and-swap
	/// folded with the delete under the fence row lock), so no fresher leader can advance the high-water
	/// between the fence check and the delete. A superseded (stale) token is rejected fail-closed with
	/// <see cref="StaleOutboxFencingTokenException"/> and nothing is deleted.
	/// </remarks>
	public async ValueTask MarkSentAsync(string messageId, long fencingToken, CancellationToken cancellationToken)
	{
		...
		// the folded block deleted nothing, and we reject fail-closed.
	...
	/// </summary>
	/// <remarks>
	/// The Oracle outbox uses a separate dead-letter table as its terminal state rather than a
	...
	public async ValueTask MarkFailedAsync(string messageId, string errorMessage, int retryCount, CancellationToken cancellationToken)
	{
		...
		// Record the failure on the main-table row: set the authoritative retry count and the last error so a
		// sub-ceiling failure is trackable by GetFailedMessages / GetStatistics without a status column
		// (design-preserving). Dead-lettering remains the terminal move only at the retry ceiling.
		...
		var failReq = new SetOutboxMessageFailed(
			messageId,
			retryCount,
			errorMessage,
			DispatcherId,
		...
		_ = await _db.Connection.ResolveAsync(failReq).ConfigureAwait(false);
	...
	/// until <paramref name="nextAttemptAt"/> has elapsed (the Oracle half of the outbox mark-failed-with-backoff path).
	/// </summary>
		...
		int batchSize, CancellationToken cancellationToken)
	{
		...
		try
		{
			// Read failed-but-retryable messages from the MAIN table (a recorded error + attempts within the
			// ceiling), NOT the terminal dead-letter table — a message failed below MaxAttempts stays in the
			// main table with its error, so the dead-letter read made sub-ceiling failures invisible.
	...
	{
		// Oracle implementation automatically deletes sent messages via MarkSentAsync So there are no sent messages to clean up in the
		// outbox table
	...
	[LoggerMessage(OutboxOracleEventId.OutboxSaveMessages, LogLevel.Debug,
		"Saving {MessageCount} outbox messages")]
	private partial void LogSaveMessages(int messageCount);

	[LoggerMessage(OutboxOracleEventId.OutboxReserveMessages, LogLevel.Debug,
	...
	[LoggerMessage(OutboxOracleEventId.OutboxConvertMessageFailed, LogLevel.Warning,
	...
	[LoggerMessage(OutboxOracleEventId.OutboxGetFailedMessagesNotSupported, LogLevel.Debug,
	...
	[LoggerMessage(OutboxOracleEventId.OutboxGetScheduledMessagesNotSupported, LogLevel.Debug,
		"Retrieving scheduled outbox messages with batchSize={BatchSize}")]
	private partial void LogGetScheduledMessages(int batchSize);

	[LoggerMessage(OutboxOracleEventId.OutboxCleanupSentMessagesNotNeeded, LogLevel.Debug,
		"CleanupAllTenantsSentMessagesAsync called but Oracle implementation automatically deletes sent messages")]
	private partial void LogCleanupSentMessagesNotNeeded();

	[LoggerMessage(OutboxOracleEventId.OutboxGetStatisticsBasic, LogLevel.Debug,
		"Retrieving outbox statistics from database")]
	private partial void LogGetStatistics();

	[LoggerMessage(OutboxOracleEventId.OutboxGetStatisticsBasic + 1, LogLevel.Debug,
