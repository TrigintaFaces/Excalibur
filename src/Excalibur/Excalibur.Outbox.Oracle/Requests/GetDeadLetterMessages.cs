...
/// <summary>
/// Represents a data request to query failed messages from the Oracle dead letter table.
/// </summary>
/// <remarks>
...
public sealed class GetDeadLetterMessages : DataRequest<IEnumerable<DeadLetterRecord>>
{
		...
		CancellationToken cancellationToken)
	{
		...
		// exact textual order their placeholders appear: :MaxRetries, then :OlderThan (when present),
		// then :Offset, then :BatchSize.
...
public sealed class DeadLetterRecord
{
	...
	/// <summary>
	/// Gets or sets the delivery priority carried on the row (<c>0</c> when the source has no priority column,
	/// e.g. the dead-letter table).
	/// </summary>
