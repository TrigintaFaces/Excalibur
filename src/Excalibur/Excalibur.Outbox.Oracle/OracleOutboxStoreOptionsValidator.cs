...
/// <summary>
/// Validates Oracle outbox store configuration options.
/// </summary>
/// <remarks>
	...
	: IValidateOptions<OracleOutboxStoreOptions>
{
	...
	/// <summary>
	/// Validates the provided Oracle outbox store options.
	/// </summary>
	/// <param name="name"> The name of the options instance being validated. </param>
	/// <param name="options"> The Oracle outbox store options to validate. </param>
	/// <returns> A validation result indicating success or failure with appropriate error messages. </returns>
	public ValidateOptionsResult Validate(string? name, OracleOutboxStoreOptions options)
	{
		// Validate that options object is not null
		if (options is null)
		{
			return ValidateOptionsResult.Fail("Oracle outbox store options cannot be null.");
		}

		// Schema name is OPTIONAL for Oracle — empty uses the connection's default schema.
		...
		// Validate schema and table names don't contain invalid characters (schema only when provided)
		if (!string.IsNullOrWhiteSpace(options.SchemaName) && !IsValidIdentifier(options.SchemaName))
		{
		...
		if (options.FailureBackoffFloorSeconds <= effectivePollSeconds)
		{
			...
			return ValidateOptionsResult.Fail(
				$"OracleOutboxStoreOptions.FailureBackoffFloorSeconds ({options.FailureBackoffFloorSeconds}s) must be " +
	...
	{
		// Oracle identifiers should start with a letter or underscore and contain only alphanumeric characters and underscores
