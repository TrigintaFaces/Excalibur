...
using Excalibur.AuditLogging.GoogleCloud;
...
public static class GoogleCloudServiceCollectionExtensions
{
	...
	private static void RegisterGoogleCloudAuditExporterCore(IServiceCollection services)
	{
		...
		var httpClientBuilder = services.AddHttpClient<GoogleCloudLoggingAuditExporter>(static (_, client) =>
		{
		...
		_ = services.AddOptions<HttpStandardResilienceOptions>(httpClientBuilder.Name)
			.Configure<IOptions<GoogleCloudAuditOptions>>(static (resilience, exporterOptions) =>
			{
		...
		_ = services.AddTransient<IAuditLogExporter>(static sp => sp.GetRequiredService<GoogleCloudLoggingAuditExporter>());
	}
}
