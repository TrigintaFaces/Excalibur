...
public sealed class PostgresComplianceOptions
{
	...
	/// <remarks>
	/// <para>
	/// The default is <see langword="false"/>: the store verifies its schema and reports an absent table at
	/// startup with its cause attached, rather than issuing DDL against a consumer's database on their behalf.
	/// Provisioning is an explicit opt-in, which is the same division the wider .NET ecosystem draws between a
	/// convenience "ensure created" call and a real migration step owned by the deployment.
	/// </para>
	/// <para>
	/// Every compliance store agrees on this default, so a consumer wiring several of them gets one first-run
	/// behaviour rather than a different one per store. Deployments that provision schema out of band — a
	/// migration tool, a DBA-owned change process, or a least-privilege runtime account without DDL rights —
	/// need no configuration to get that.
	/// </para>
	/// </remarks>
...
public sealed partial class PostgresComplianceStore : IComplianceStore, IDisposable
{
	...
	/// <summary>
	/// Gets the tenant scope this store reads and writes under, resolved in one place so every
	...
	private TenantScope CurrentTenantScope =>
		_requireTenant ? TenantScope.FromContext(_tenantContext) : TenantScope.Untenanted;
	...
	public PostgresComplianceStore(
		...
		: this(CreateConnectionFactory(options?.Value), options?.Value, tenantContext, tenantContextOptions, logger)
	{
	}
	...
	/// <param name="options">The Postgres compliance options.</param>
	/// <param name="tenantContext">
	/// The ambient tenant context. Required: the store resolves its scope from here in multi-tenant mode,
	/// and a single-tenant host receives the framework default context.
	/// </param>
	/// <param name="tenantContextOptions">
	/// The tenant-context options. Its <see cref="TenantContextOptions.RequireTenant"/> (set by
	...
	private async Task VerifySchemaExistsAsync(NpgsqlConnection connection, CancellationToken cancellationToken)
	{
		...
		// A probe that asks only which tables are present reports healthy on precisely the database that is
		// broken: one provisioned before a column was added, where the table is there and the wrong shape.
		// The consumer then gets a dead store plus a check that told them it was fine. Automatic schema
		// creation does not repair it either -- CREATE TABLE IF NOT EXISTS only creates absent tables, so a
		// table that exists with a stale column set is skipped by the very path offered as the remedy.
		//
		// One query for all three tables, so a consumer whose schema is behind in several places learns
		// every gap at once rather than fixing them one restart at a time.
		//
		// information_schema returns only objects the connecting role has privileges on. That direction is
		// safe here: a column it cannot see reads as missing and the check FAILS, which is the correct
		// outcome for a role that cannot use the column anyway.
		const string Sql = """
		...
		var rows = await connection.QueryAsync<(string TableName, string ColumnName)>(command).ConfigureAwait(false);

		var present = new Dictionary<string, HashSet<string>>(StringComparer.Ordinal);
		foreach (var (table, column) in rows)
		{
			if (!present.TryGetValue(table, out var columns))
			{
				columns = new HashSet<string>(StringComparer.Ordinal);
				present[table] = columns;
			}
		...
		{
			if (!present.TryGetValue(tableName, out var actualColumns))
			{
			...
			var missingColumns = requiredColumns.Where(c => !actualColumns.Contains(c)).ToList();
			if (missingColumns.Count > 0)
			{
				staleTables.Add($"{tableName} (missing {string.Join(", ", missingColumns)})");
			}
		}
		...
		if (missingTables.Count > 0)
		{
			throw new InvalidOperationException(
				$"The compliance schema '{_options.SchemaName}' is missing required table(s): {string.Join(", ", missingTables)}. " +
		...
		{
			throw new InvalidOperationException(
				$"The compliance schema '{_options.SchemaName}' has table(s) whose column set is behind this store's "
				+ $"statements: {string.Join("; ", staleTables)}. Enabling automatic schema creation will NOT repair "
				+ "this, because that path only creates tables that are absent. Migrate the listed table(s), then restart.");
		}
	}
	...
	/// </summary>
	/// <remarks>
	/// Kept beside the statements it mirrors: a column added to one of the writes above without a line here
	/// is a column the verification stops covering, which returns this check to the table-existence
	...
	[
		...
		[
			"tenant_id", "subject_id", "purpose", "granted_at", "expires_at", "legal_basis",
