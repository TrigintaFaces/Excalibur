...
public class ConflictException : ResourceException
{
	...
	public ConflictException()
		: base(409, "A conflict occurred with the current state of the resource.", null)
	{
	}
	...
	{
		var ex = new ConflictException(FormatConflictMessage(resource, reason))
		{
			Resource = resource,
			Reason = reason,
		};
	...
	{
		var ex = new ConflictException($"A {resource} with identifier '{identifier}' already exists.")
		{
			Resource = resource,
		};
		_ = ex.WithContext("resource", resource);
		_ = ex.WithContext("identifier", identifier);
		return ex;
	}
	...
	{
		var ex = new ConflictException($"Cannot transition from '{currentState}' to '{targetState}'.")
		{
			Resource = resource,
		};
