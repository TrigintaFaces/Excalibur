...
/// <summary>
/// Builds a composed <see cref="IPersistenceProvider"/> by stacking
/// <see cref="DelegatingPersistenceProvider"/> decorators around an inner provider.
/// Follows the <c>ChatClientBuilder</c> pattern from <c>Microsoft.Extensions.AI</c>.
/// </summary>
/// <remarks>
/// <para>
/// Decorators are applied in registration order: the first registered decorator is the
/// outermost wrapper. This is identical to the ordering semantics of
/// <c>TransportSenderBuilder</c> and <c>HttpMessageHandlerBuilder</c>.
/// </para>
/// <para>
/// Example: composing logging and retry decorators around a SQL Server provider:
/// </para>
/// <code>
/// var provider = new PersistenceProviderBuilder(sqlServerProvider)
///     .Use(inner => new RetryPersistenceProvider(inner, retryPolicy))
///     .Use(inner => new LoggingPersistenceProvider(inner, logger))
///     .Build();
/// </code>
/// </remarks>
