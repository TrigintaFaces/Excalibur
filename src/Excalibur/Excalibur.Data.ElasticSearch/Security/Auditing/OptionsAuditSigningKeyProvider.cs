...
/// <see cref="AuditOptions"/>. It performs no key generation: when no key is configured it fails
/// closed (throws on the write path, returns <see langword="null"/> on the verify path) so audit
...
internal sealed class OptionsAuditSigningKeyProvider : IAuditSigningKeyProvider
{
	...
	/// </summary>
	/// <param name="options">The audit options carrying the optional signing key and key identifier.</param>
	public OptionsAuditSigningKeyProvider(IOptions<AuditOptions> options) => _options = options;
	...
	public ValueTask<(string KeyId, byte[] Key)> GetCurrentSigningKeyAsync(CancellationToken cancellationToken)
	{
		...
		var key = configuration.IntegritySigningKey;
		if (key is null || key.Length == 0)
		{
			throw new InvalidOperationException(
				"Audit log integrity is enabled but no signing key is configured. Set " +
				$"{nameof(AuditOptions)}.{nameof(AuditOptions.IntegritySigningKey)} (sourced from a secret " +
				$"manager / KMS) or register a custom {nameof(IAuditSigningKeyProvider)}.");
		}

		return ValueTask.FromResult((configuration.IntegrityKeyId, key));
	}
	...
	public ValueTask<byte[]?> GetSigningKeyAsync(string keyId, CancellationToken cancellationToken)
	{
		...
		var key = configuration.IntegritySigningKey;
		if (key is null || key.Length == 0
			|| !string.Equals(keyId, configuration.IntegrityKeyId, StringComparison.Ordinal))
		{
