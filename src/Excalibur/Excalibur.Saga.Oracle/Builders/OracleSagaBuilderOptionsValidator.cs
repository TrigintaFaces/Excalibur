...
/// <summary>
/// Validates <see cref="OracleSagaStoreOptions"/> at startup via ValidateOnStart.
/// Ensures a connection has been configured through the builder.
/// </summary>
internal sealed class OracleSagaBuilderOptionsValidator : IValidateOptions<OracleSagaStoreOptions>
{
	internal bool HasBuilderConnection { get; init; }

	/// <inheritdoc/>
	public ValidateOptionsResult Validate(string? name, OracleSagaStoreOptions options)
	{
		if (!HasBuilderConnection && string.IsNullOrWhiteSpace(options.ConnectionString))
		{
			return ValidateOptionsResult.Fail(
				...
				"or BindConfiguration() inside UseOracle().");
		}
