...
internal sealed class OracleSagaTimeoutStoreOptionsValidator : IValidateOptions<OracleSagaTimeoutStoreOptions>
{
	public ValidateOptionsResult Validate(string? name, OracleSagaTimeoutStoreOptions options)
	{
		if (options is null)
		{
			return ValidateOptionsResult.Fail("Oracle saga timeout store options cannot be null.");
		}
