...
internal sealed class SqlServerDeadLetterQueueOptionsValidator : IValidateOptions<SqlServerDeadLetterQueueOptions>
{
	public ValidateOptionsResult Validate(string? name, SqlServerDeadLetterQueueOptions options)
	{
		if (options is null)
		{
			return ValidateOptionsResult.Fail("SQL Server dead letter queue options cannot be null.");
		}
