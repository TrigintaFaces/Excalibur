...
public interface IGrantRepository : IEventSourcedRepository<Grant>
{
	...
	Task<IEnumerable<Grant>> MatchingAsync(GrantScope scope, string? userId, CancellationToken cancellationToken);
	...
	/// <exception cref="ArgumentException"> Thrown if <paramref name="userId" /> is null or empty. </exception>
	Task<IEnumerable<Grant>> ReadAllAsync(string userId, CancellationToken cancellationToken);
}
