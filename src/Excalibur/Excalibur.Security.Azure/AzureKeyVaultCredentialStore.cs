...
public sealed partial class AzureKeyVaultCredentialStore : IWritableCredentialStore
{
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="AzureKeyVaultCredentialStore" /> class
	/// with an explicit <see cref="ISecretClient"/>. Used by tests via
	/// <c>InternalsVisibleTo</c>; not part of the public contract.
	/// </summary>
	/// <param name="configuration"> The configuration containing Key Vault settings. </param>
	/// <param name="logger"> The logger instance. </param>
	/// <param name="secretClient">
	/// Optional secret client seam. When <see langword="null"/> a default
	/// <see cref="SecretClientAdapter"/> wrapping a real
	/// <see cref="SecretClient"/> authenticated via
	/// <see cref="DefaultAzureCredential"/> is constructed from the configured
	/// vault URI.
	/// </param>
		...
		ISecretClient? secretClient)
	{
		...
		else
		{
			...
			_secretClient = new SecretClientAdapter(new SecretClient(new Uri(vaultUri), credential));
		}
	...
	[LoggerMessage(AzureSecurityEventId.AzureKeyVaultCredentialStoreCreated, LogLevel.Information, "Azure Key Vault credential store initialized with vault {VaultUri}")]
	private partial void LogKeyVaultInitialized(string vaultUri);

	[LoggerMessage(AzureSecurityEventId.AzureKeyVaultSecretNotFound, LogLevel.Debug, "Secret {Key} not found in Azure Key Vault")]
	private partial void LogSecretNotFound(string key);

	[LoggerMessage(AzureSecurityEventId.AzureKeyVaultRetrieved, LogLevel.Debug, "Secret {Key} retrieved from Azure Key Vault")]
	private partial void LogSecretRetrieved(string key);

	[LoggerMessage(AzureSecurityEventId.AzureKeyVaultRequestFailed, LogLevel.Debug, "Secret {Key} not found in Azure Key Vault (404)")]
	private partial void LogSecretNotFound404(string key);

	[LoggerMessage(AzureSecurityEventId.AzureKeyVaultRetrieveFailed, LogLevel.Error, "Failed to retrieve secret {Key} from Azure Key Vault")]
	private partial void LogRetrievalFailed(Exception ex, string key);

	[LoggerMessage(AzureSecurityEventId.AzureKeyVaultStored, LogLevel.Information, "Secret {Key} stored in Azure Key Vault")]
	private partial void LogSecretStored(string key);

	[LoggerMessage(AzureSecurityEventId.AzureKeyVaultStoring, LogLevel.Error, "Failed to store secret {Key} in Azure Key Vault")]
