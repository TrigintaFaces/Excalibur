...
/// <summary>
/// Extension methods for registering CDC data change handlers from assemblies.
/// </summary>
public static class DataChangeHandlerServiceCollectionExtensions
{
	/// <summary>
	/// Scans the specified assembly for classes implementing <see cref="IDataChangeHandler"/>
	/// and registers them in the service collection.
	/// </summary>
	/// <param name="services">The service collection.</param>
	/// <param name="assembly">The assembly to scan for data change handler implementations.</param>
	...
	/// <see cref="IDataChangeHandler"/> and registers them with the DI container.
	/// Each handler is registered both as its concrete type and as <see cref="IDataChangeHandler"/>.
	/// </para>
	/// <para>
	...
	/// <example>
	/// <code>
	/// // Register all data change handlers from the current assembly
	/// services.AddDataChangeHandlersFromAssembly(typeof(Program).Assembly);
	///
	/// // Register with transient lifetime
	/// services.AddDataChangeHandlersFromAssembly(typeof(Program).Assembly, ServiceLifetime.Transient);
	/// </code>
	/// </example>
	[RequiresUnreferencedCode("Assembly scanning uses reflection to discover types implementing IDataChangeHandler.")]
	public static IServiceCollection AddDataChangeHandlersFromAssembly(
		...
		ServiceLifetime lifetime = ServiceLifetime.Singleton)
	{
		...
		foreach (var handlerType in handlerTypes)
		{
			...
			// Register as IDataChangeHandler for enumerable resolution
			services.TryAddEnumerable(new ServiceDescriptor(
				typeof(IDataChangeHandler), handlerType, lifetime));
		}
