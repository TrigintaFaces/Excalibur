...
/// <summary>
/// Registers the singleton Kubernetes election and the outbox leader gate that fences on it, for a consumer
/// who has named the lease to contend for.
/// </summary>
/// <remarks>
/// <para>
/// The Kubernetes provider registers an <see cref="ILeaderElectionFactory"/>, which is the right shape for a
...
/// </para>
/// <para>
/// The election is registered only when the consumer has named the lease. A Lease is a namespace-scoped
/// Kubernetes object, so a framework-chosen default name would put two unrelated workloads in the same
/// namespace into contention for one lease — each stalling the other's outbox. Naming is the consumer's
/// call, exactly as the SQL Server provider requires an explicit lock resource. When no name is given the
/// registration stays factory-only, which is what it has always been, so nothing that works today changes
/// behaviour.
/// </para>
/// </remarks>
internal static class KubernetesElectionRegistration
{
	internal static void RegisterSingletonElectionAndGate(IServiceCollection services, string? resourceName)
	{
		if (string.IsNullOrWhiteSpace(resourceName))
		{
			// The consumer never named a lease, so there is no singleton election to register and no safe
			// name to invent. Elections remain available through the factory.
			return;
		}

		services.TryAddKeyedSingleton<ILeaderElection>("kubernetes", (sp, _) =>
		...
		services.TryAddKeyedSingleton<ILeaderElection>("default", (sp, _) =>
			sp.GetRequiredKeyedService<ILeaderElection>("kubernetes"));
